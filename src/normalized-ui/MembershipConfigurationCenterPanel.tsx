...
export function MembershipConfigurationCenterPanel({api,auth}:{api:NormalizedApiClient;auth:StaffAuthView}){
  ...
  const [references,setReferences]=useState<ConfigurationReference[]|null>(null)
  const [referenceError,setReferenceError]=useState('')
  const loadReferences=useCallback(async()=>{
    try { const response=await api.getEndpoint<{data:ConfigurationReference[]}>('/api/staff/loyalty/configuration-center/references');setReferences(response.data);setReferenceError('') }
    catch { setReferences(null);setReferenceError('商品、权益和活动选项暂未读到，请重新读取后再修改关联。') }
  },[api])
...
function Field({fieldKey,value,onChange,references}:{fieldKey:string;value:unknown;references:ConfigurationReference[]|null;onChange(value:unknown):void}){
  ...
  if(options)return <label>{label}<select value={String(value)} onChange={(event)=>onChange(event.target.value)}>{!options.some(([option])=>option===String(value))&&<option value={String(value)} disabled>原设置待核对</option>}{options.map(([option,text])=><option key={option} value={option}>{text}</option>)}</select></label>
