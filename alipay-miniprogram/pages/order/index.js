...
const { requestAlipaySubscription, mergeAlipayNotificationPromptOptions, extractPromptPresentation } = require('../../utils/alipay-subscription')
...
// The shared server currently creates another platform's payment actions only. Keep the
// Alipay checkout fail-closed until it can return a server-created tradeNO.
function alipayOnlinePaymentEnabled() {
  return getRuntimeConfig().alipayPaymentEnabled === true
}
...
function applyAlipaySubscriptionOutcomes(options, outcomes) {
...
Page({
  ...
  data: {
    ...
    alipayOrderSelectionPromptOptions: [],
  },
  ...
  queuePendingGuestPaymentAbandonment(pendingOverride) {
    ...
    const pendingPayment = pendingOverride || this.data.pendingPayment || runtime.getStorageSync(PENDING_PAYMENT_KEY) || null
    ...
    runtime.setStorageSync(PENDING_PAYMENT_KEY, retained)
  ...
  stopShakeRecommendation() {
    ...
    if (this.shakeListener && runtime.offAccelerometerChange) runtime.offAccelerometerChange(this.shakeListener)
    this.shakeListener = null
    if (runtime.stopAccelerometer) runtime.stopAccelerometer({ fail: () => {} })
  ...
  scheduleSharedCartPoll(delay, request) {
    ...
    this.sharedCartTimer = setTimeout(async () => {
      ...
      const nextDelay = Math.min(60000, 5000 * Math.pow(2, Math.min(this.sharedCartPollFailures, 4)))
  ...
  async preparePage(silent) {
    ...
    try {
      ...
      ;[PENDING_PAYMENT_KEY, PENDING_GUEST_PAYMENT_ABANDONMENT_KEY, CHECKOUT_ATTEMPT_KEY].forEach(key => {
        const stored = runtime.getStorageSync(key)
        if (stored && stored.tableScope !== confirmedScope) runtime.removeStorageSync(key)
      })
  ...
  async loadActiveData(request) {
    ...
    const paymentScope = tableSessionCacheScope()
    let storedAbandonment = runtime.getStorageSync(PENDING_GUEST_PAYMENT_ABANDONMENT_KEY) || null
    if (storedAbandonment && storedAbandonment.tableScope !== paymentScope) {
      runtime.removeStorageSync(PENDING_GUEST_PAYMENT_ABANDONMENT_KEY)
      storedAbandonment = null
    }
    let storedPending = runtime.getStorageSync(PENDING_PAYMENT_KEY) || null
    ...
    const pendingPayment = storedPending
      ? Object.assign({}, storedPending, {
          canContinue: Boolean(!storedPending.alipayAcceptedAt
          ...
          statusText: tableOrdersAvailable && storedOrder
            ? storedOrder.paymentAccess === 'status_review'
              ? '付款结果确认中，请勿重复支付'
              : storedPending.alipayAcceptedAt
                ? '支付宝支付已完成，到账确认中，请勿重复支付'
    ...
    const pendingCheckout = (() => {
      const stored = runtime.getStorageSync(CHECKOUT_ATTEMPT_KEY)
  ...
  async loadOrderExtra(name, request) {
    ...
    const loaders = {
      ...
      order_checkout: async () => ({ alipayNotificationPromptOptions: extractPromptPresentation(await getAlipayNotificationPrompt('order_checkout')) }),
      order_selection: async () => ({ alipayOrderSelectionPromptOptions: extractPromptPresentation(await getAlipayNotificationPrompt('order_selection')) }),
  ...
  dismissMembershipInvite() {
    ...
    const cooldownHours = Number.isFinite(configuredHours) && configuredHours >= 1 && configuredHours <= 2160
      ? configuredHours : 24
    runtime.setStorageSync(MEMBERSHIP_INVITE_DISMISSED_KEY, Date.now() + cooldownHours * 60 * 60 * 1000)
    this.setData({ membershipInviteVisible: false })
  },

  openMembershipRecovery() { runtime.switchTab({ url: '/pages/profile/index' }) },
  openPrivacy() { runtime.navigateTo({ url: '/pages/privacy/index' }) },

  showMembershipTerms() {
    if (!this.data.membershipTerms) return
    runtime.navigateTo({ url: '/pages/membership-terms/index?source=mini_menu&action=view' })
  },
  ...
  enrollFromMenu() {
    ...
    runtime.navigateTo({ url: '/pages/membership-terms/index?source=mini_menu&action=enroll' })
  },
  ...
  previewProductImage() {
    ...
    if (!imageUrl) return
    runtime.previewImage({ current: imageUrl, urls: [imageUrl] })
  },
  ...
  onShakeRecommendation() {
    ...
    if (!runtime.startAccelerometer || !runtime.onAccelerometerChange) return this.recommend('shake', tableRequest)
    ...
    const refresh = () => {
      ...
      completed = true
      this.stopShakeRecommendation()
      if (runtime.vibrateShort) runtime.vibrateShort({ type: 'light', fail: () => {} })
    ...
    runtime.startAccelerometer({
      interval: 'game',
      success: () => {
        runtime.onAccelerometerChange(this.shakeListener)
  ...
  async rejectRecommendation(event) {
    ...
    const selectedIndex = await new Promise((resolve) => runtime.showActionSheet({
  ...
  async clearCart() {
    ...
    try {
      ...
      if (!writeGuard.isCurrentWrite(write)) return
      this.updateCart(sharedCartView(sharedCart, this.data.products), sharedCart)
      runtime.showToast({ title: '已清空本桌购物车', icon: 'none' })
    } catch (error) {
      if (!writeGuard.isCurrentWrite(write)) return
  ...
  async removeCartLine(event) {
    ...
    try {
      ...
      if (!writeGuard.isCurrentWrite(write)) return
      this.updateCart(sharedCartView(sharedCart,this.data.products),sharedCart)
      runtime.showToast({ title:'已移除这件商品',icon:'none' })
    } catch (error) {
      if (!writeGuard.isCurrentWrite(write)) return
      if (error&&error.code==='SHARED_CART_VERSION_CONFLICT') {
        await this.refreshSharedCart(true,this.currentTableRequest())
  ...
  openAccount() { runtime.navigateTo({ url: '/pages/account/index' }) },
  openBenefits() { runtime.switchTab({ url: '/pages/profile/index' }) },
  ...
  async retryCheckout(request) {
    ...
    const attempt = runtime.getStorageSync(CHECKOUT_ATTEMPT_KEY)
  ...
  async handlePaymentAction(action, request) {
    ...
    if (!isPresentableAlipayTradeAction(action)) {
      ...
      this.setData({
        paymentResult: {
          ...
          title: waitingForResult ? '正在核对付款结果' : '未能发起支付宝支付',
    ...
    try {
      ...
      // before calling Alipay so onHide/onUnload never mistake that sheet for
      ...
      runtime.setStorageSync(PENDING_PAYMENT_KEY, presentingPayment)
      this.setData({ pendingPayment: presentingPayment })
      await new Promise((resolve, reject) => runtime.requestPayment({
      ...
      const pendingPayment = Object.assign({}, this.data.pendingPayment, {
        ...
        canContinue: false,
        paymentPresentationState: 'alipay_accepted',
        paymentPresentationInFlight: false,
        alipayAcceptedAt: new Date().toISOString(),
      })
      runtime.setStorageSync(PENDING_PAYMENT_KEY, pendingPayment)
      this.setData({ pendingPayment, success: '' })
      // Keep notification handling after the payment callback so checkout
      // remains one uninterrupted confirm -> password flow. A notification
      // failure must never rewrite a successful payment as a payment failure.
  ...
  async restoreResolvedPayment(action, tableRequest) {
    ...
    runtime.setStorageSync(PENDING_PAYMENT_KEY, pendingPayment)
    const checkout = runtime.getStorageSync(CHECKOUT_ATTEMPT_KEY)
    if (checkout && checkout.tableScope === original.tableScope) runtime.removeStorageSync(CHECKOUT_ATTEMPT_KEY)
    const abandonment = runtime.getStorageSync(PENDING_GUEST_PAYMENT_ABANDONMENT_KEY)
    ...
    read.promise = (async () => {
      try {
        ...
        runtime.setStorageSync(PENDING_PAYMENT_KEY, resolvedPayment)
  ...
  async confirmPaymentOutcome(pendingPayment, request) {
    ...
    const waitingPayment = Object.assign({}, pendingPayment, {
      statusText: '支付宝支付已完成，到账确认中',
      canContinue: false,
    })
    runtime.setStorageSync(PENDING_PAYMENT_KEY, waitingPayment)
    this.setData({
  ...
  async offerOrderNotifications(context, request) {
    if (request && !this.isCurrentTableRequest(request)) return
    if (context === 'order_selection' && !(this.data.alipayOrderSelectionPromptOptions || []).length) return
  ...
  async ensureBalanceNotificationAuthorizations(context, request) {
    ...
    let options = context === 'order_selection'
      ? this.data.alipayOrderSelectionPromptOptions
      : this.data.alipayNotificationPromptOptions
    if (context === 'order_selection' && (!options || !options.length)) return
    if (!options || !options.length) {
      try {
        const prompts = await Promise.all([
          getAlipayNotificationPrompt(context === 'order_selection' ? 'order_selection' : 'order_checkout'),
          getAlipayNotificationPrompt('coupon_open'),
          getAlipayNotificationPrompt('member_card'),
        ])
        options = mergeAlipayNotificationPromptOptions(
    ...
    const result = await requestAlipaySubscription(options || [])
    if (!this.isCurrentTableRequest(tableRequest) || !result.outcomes.length) return
    this.setData({
      alipayNotificationPromptOptions: applyAlipaySubscriptionOutcomes(
        this.data.alipayNotificationPromptOptions, result.outcomes,
      ),
      alipayOrderSelectionPromptOptions: applyAlipaySubscriptionOutcomes(
        this.data.alipayOrderSelectionPromptOptions, result.outcomes,
      ),
    })
  },
