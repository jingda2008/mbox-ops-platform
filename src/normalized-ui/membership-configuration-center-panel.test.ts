...
describe('membership configuration center operating contract',()=>{
  ...
  it('retires every former client-certified approval request',()=>{
    ...
    for(const domain of ['base_points','tier_policy','tier_benefits','redemption_catalog','promotion_points','membership_terms']) expect(former).toContain(`openMembershipConfiguration('${domain}'`)
    expect(former).toContain('前往配置中心审批')
    expect(panel).toContain("query.get('configuration')")
    expect(panel).toContain("item.configurationId===id&&item.domain===domain")
  })
})
