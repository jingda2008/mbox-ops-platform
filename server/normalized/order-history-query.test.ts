...
integration('order history SQL scope and session identity',()=>{
 ...
 let currentDate:string,oldDate:string,earliestDate:string
 beforeAll(async()=>{
  ...
  currentDate=await transactions.run(scope,async tx=>(await tx.query<{date:string}>('SELECT mbox.current_operating_business_date($1,$2)::text AS date',[scope.tenantId,scope.storeId])).rows[0]!.date)
  const previous=(days:number)=>new Date(Date.parse(`${currentDate}T00:00:00Z`)-days*86400000).toISOString().slice(0,10)
  oldDate=previous(9);earliestDate=previous(2)
  ...
  for(const [index,date,payment] of [[1,oldDate,'paid'],[2,oldDate,'unpaid'],[3,currentDate,'paid']] as const){
 ...
 const read=(extra:Record<string,unknown>={})=>transactions.run(scope,tx=>readOperatingHistory(tx,{
  businessDate:oldDate,endDate:currentDate,table:'',employee:'',page:0,
  earliestBusinessDate:earliestDate,allowFinancialSummary:false,...extra,
 ...
 it('keeps older unresolved orders in the current-day view and export',async()=>{
  const result=await read({businessDate:currentDate,exportAll:true})
 ...
 it('enforces fresh account permissions and ignores caller-supplied scope overrides',async()=>{
  ...
  const filter={businessDate:oldDate,endDate:currentDate,table:'',employee:'',page:0,earliestBusinessDate:null,allowFinancialSummary:true}
 ...
 it('requires independent print authority, retains history scope and replays one bill request without duplicating paper jobs',async()=>{
  ...
  await app.register(hardwareApiPlugin,{transactions,commands:new NormalizedCommandExecutor(transactions),
   resolveContext:()=>({scope,employeeId:reader,businessDate:currentDate,capabilities:['order.history.view','order.bill.print']})})
  try{
