 ...
 app.get('/payments/finance-review',async(request,reply)=>{
  ...
  const data=await options.transactions.run(context.scope,async tx=>{
   ...
   return (await tx.query(`SELECT p.id,p.public_id AS "publicId",p.amount_minor::text AS "amountMinor",p.status,p.created_at::text AS "createdAt",
    ...
    o.public_id AS "orderPublicId",t.code AS "tableCode",financial.signals AS "financialSignals"
    ...
    LEFT JOIN LATERAL(SELECT array_agg(DISTINCT signal.signal) AS signals FROM mbox.payment_financial_monitoring_signals signal
      WHERE signal.tenant_id=p.tenant_id AND signal.store_id=p.store_id AND (signal.subject_id=p.id OR EXISTS(SELECT 1 FROM mbox.order_payment_facts fact WHERE fact.tenant_id=p.tenant_id AND fact.store_id=p.store_id AND fact.id=p.id AND fact.order_id=signal.subject_id))
        AND signal.signal IN ('order_overcollected','cancelled_order_captured','succeeded_payment_missing_reconciliation')) financial ON true
    ...
    ORDER BY p.created_at,p.id LIMIT 101 OFFSET $3`,[context.scope.tenantId,context.scope.storeId,page*100])).rows
  },{readOnly:true})
  return reply.send({data:data.slice(0,100),hasMore:data.length>100})
 })
