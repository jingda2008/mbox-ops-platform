...
export class ItemAfterSalesQuery {
  ...
  item(input:{scope:Readonly<StoreScope>;employeeId:string;itemId:string;staffSessionId?:string;deviceAccessLeaseId?:string}){
    return this.transactions.run(input.scope,async tx=>{
      ...
      const notices=(await tx.query<{id:string;case_id:string;station_code:string;instruction:string;created_at:string;source_status:string|null;printed:boolean;print_failed:boolean}>(`SELECT notice.id,notice.case_id,notice.station_code,notice.instruction,notice.created_at::text,source.status AS source_status,
        ...
        FROM mbox.item_after_sales_notices notice LEFT JOIN mbox.print_source_jobs source ON source.tenant_id=notice.tenant_id AND source.store_id=notice.store_id AND source.source_outbox_message_id=notice.source_outbox_message_id
      ...
      return {quantityEntryUnavailableReason,redeliveries,redeliveryAvailableQuantity,canConfirmRedelivery,canCancelRedelivery,
        ...
        units,cases:progress.map(record=>{const original=cases.find(value=>value.id===record.caseId)!
