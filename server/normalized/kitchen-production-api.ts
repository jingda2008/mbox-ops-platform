...
import {kitchenCompatibilityKey,type KitchenCommand,type KitchenCommandResult,type KitchenDestination} from '../../src/shared/kitchen-production.js'
...
import {readKitchenBatches,readKitchenBoard,readKitchenSources} from './kitchen-production-query.js'
import type {ScopedTransaction} from './transaction-runner.js'
import {ItemQuantityRepository} from './item-quantity-repository.js'

type Options=Pick<CommerceKdsApiOptions,'resolveContext'|'staffAccessTransactions'|'commandExecutor'|'createKdsRepository'|'createOrderRepository'>&{enabled?:boolean}
...
export const kitchenProductionApiPlugin:FastifyPluginAsync<Options>=async(app,options)=>{
  app.get('/commerce/kitchen-board',async(request,reply)=>handleCommerceRoute(reply,async()=>{
    ...
    const data=await options.staffAccessTransactions.run(context.scope,tx=>readKitchenBoard(tx,context,options.enabled===true),{isolation:'repeatable-read',readOnly:true})
  ...
  app.post('/commerce/kitchen-board/commands',async(request,reply)=>handleCommerceRoute(reply,async()=>{
    ...
    const command=parseKitchenCommand(body.command)
    ...
    await options.staffAccessTransactions.run(context.scope,tx=>authorize(tx,context))
    const operationKey=createHash('sha256').update(`${context.employeeId}:${key}`).digest('hex')
    const execution=await options.commandExecutor.execute({scope:context.scope,operationScope:'commerce.kitchen.batch',
      idempotencyKey:operationKey,requestFingerprint:JSON.stringify({employeeId:context.employeeId,command}),resultCodec:codec},
    tx=>executeKitchenCommand(tx,options,context,command,key,request.id))
    return reply.send({data:execution.value,replayed:execution.replayed})
  }))
}

async function authorize(tx:ScopedTransaction,context:CommerceKdsRequestContext){
  // Production authorization is scoped by station; table assignment is checked only for manager exceptions.
  await new NormalizedKdsAuthorization().assertCanActOnTask({transaction:tx,...context,action:'start',stationCode:'kitchen',tableId:''})
}

export async function executeKitchenCommand(tx:ScopedTransaction,options:Options,context:CommerceKdsRequestContext,command:KitchenCommand,key:string,requestId:string):Promise<CommandOutcome<KitchenCommandResult>> {
  await authorize(tx,context)
  const scope=[tx.scope.tenantId,tx.scope.storeId]
  const receipt=(await tx.query<{same:boolean;result:KitchenCommandResult}>(`SELECT request_body=$5::jsonb AS same,result FROM mbox.kitchen_production_receipts WHERE tenant_id=$1 AND store_id=$2 AND employee_id=$3 AND operation_key=$4`,[...scope,context.employeeId,key,JSON.stringify(command)])).rows[0]
  ...
  let batchId:string,quantity=0,released=false
  const eventKey=createHash('sha256').update(`${context.employeeId}:${key}`).digest('hex')
  const transition=async(taskId:string,action:'start'|'complete',count:number,unitIds?:string[])=>{
    const outcome=await performKdsAction(tx,{...options,quantityActionsEnabled:true},context,taskId,action,`kitchen:${eventKey}`,requestId,null,count,unitIds)
    outcomes.push(outcome)
    return outcome.result.affectedUnitIds!
  }
  if(command.action==='start'||command.action==='quick-ready'){
    if(options.enabled!==true)conflict('新增制作批次已暂停，已有批次仍可出锅、分盘或恢复原结果','KITCHEN_ADMISSION_PAUSED')
    ...
    const inserted=await tx.query<{id:string}>(`INSERT INTO mbox.kitchen_production_batches
      (id,tenant_id,store_id,product_id,product_name,specification,item_note,order_note,created_by_employee_id,started_at,anchor_at,equipment,released_at,expected_seconds,original_quantity)
      VALUES($3,$1,$2,$4,$5,$6,$7,$8,$9,CASE WHEN $10 THEN clock_timestamp() END,$11,$12,CASE WHEN $13 THEN clock_timestamp() END,$14,$15)
      ON CONFLICT(tenant_id,store_id,equipment_key) WHERE equipment_key IS NOT NULL AND released_at IS NULL DO NOTHING RETURNING id`,
    [...scope,batchId,first.productId,first.productName,first.specification,first.itemNote,first.orderNote,context.employeeId,command.action==='start',chosen.map(item=>item.orderCreatedAt).sort()[0],command.equipment,released,command.expectedSeconds,quantity])
    ...
    const before=(await readKitchenBatches(tx,batchId))[0]
    ...
    const batch=(await readKitchenBatches(tx,batchId))[0]!
    if(batch.employeeId!==context.employeeId)conflict(`该批由${batch.employeeName}制作，请由原员工核对`,'KITCHEN_OWNER_CHANGED')
    ...
    if(command.action==='ready'){
      ...
      const after=(await readKitchenBatches(tx,batchId))[0]!
  ...
  const result:KitchenCommandResult={batchId,action:command.action,quantity,released}
  await tx.query(`INSERT INTO mbox.kitchen_production_receipts(tenant_id,store_id,employee_id,operation_key,request_body,result) VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb)`,[...scope,context.employeeId,key,JSON.stringify(command),JSON.stringify(result)])
  return {result,auditEvents:[...outcomes.flatMap(outcome=>outcome.auditEvents),{actor:{type:'employee',employeeId:context.employeeId},action:`kitchen.${command.action}`,objectType:'kitchen_production_batch',objectId:batchId,businessDate:context.businessDate,requestId,afterData:{...result,command:command as unknown as JsonValue}}],outboxMessages:outcomes.flatMap(outcome=>outcome.outboxMessages)}
}
...
export function parseKitchenCommand(value:unknown):KitchenCommand {
  ...
  if(body.action==='release')return {action:'release',batchId:uuid(body.batchId)}
  if(body.action==='ready')return {action:'ready',batchId:uuid(body.batchId),items:selections(body.items,true).map(({quantity:_quantity,expectedUnmade:_expectedUnmade,...item})=>item)}
