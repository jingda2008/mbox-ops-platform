...
async function management(page:Page){
  await page.goto('/staff/member-management#work=member-rules');await page.getByRole('button',{name:/会员经营配置中心/}).click()
  return page.getByRole('region',{name:'累计签到奖励',exact:true})
}
...
test('configurable attendance rewards require individual or daily batch approval and survive a lost reply',async({page},testInfo)=>{
  ...
  for(const member of ['MBX-REWARD-A','MBX-REWARD-B','MBX-REWARD-C'])await visit(page,member)
  panel=await management(page);await panel.getByRole('button',{name:'读取签到奖励',exact:true}).click()
  ...
  let lost=false
  await page.route('**/api/staff/member-visit-rewards',async route=>{
  ...
  await first.getByRole('button',{name:'单独批准并发券',exact:true}).click();await confirm(page)
  ...
  await panel.getByRole('button',{name:'批量批准并发券（2）'}).click();await confirm(page)
  ...
  await page.goto('/staff/tasks');await page.getByLabel('输入会员号或核销码').fill('MBX-REWARD-A');await page.getByRole('button',{name:'查询活动与权益',exact:true}).click()
  await page.getByRole('group',{name:'选择签到方式'}).getByRole('button',{name:'仅到店签到',exact:true}).click()
  await expect(page.getByRole('region',{name:'会员活动与权益查询'})).toContainText('测试累计签到赠品')
  const visits=page.getByRole('region',{name:'仅到店签到',exact:true});await expect(visits).toContainText('已发券 1 轮')
  await visits.getByRole('button',{name:'撤回误签到'}).click();await page.getByRole('alertdialog').getByRole('button',{name:'确认撤回',exact:true}).click()
  await visits.getByRole('button',{name:'确认到店签到',exact:true}).click();await expect(visits).toContainText('待审批 0 轮，已发券 1 轮')
  panel=await management(page);await panel.getByRole('combobox',{name:'审批状态',exact:true}).selectOption('issued');await panel.getByRole('button',{name:'读取签到奖励',exact:true}).click()
