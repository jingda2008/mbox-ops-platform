...
Page({
  ...
  openPage(event) { runtime.navigateTo({ url: event.currentTarget.dataset.url }) },
  openTab(event) { runtime.switchTab({ url: event.currentTarget.dataset.url }) },

  openTonightSchedule() {
    const performance = this.data.performance
    if (!performance || !(performance.schedules && performance.schedules.length)) {
      runtime.showToast({ title: '今晚暂无演出安排', icon: 'none' })
      return
    }
  ...
  openPerformerProfile() {
    ...
    runtime.navigateTo({ url: '/pages/performances/index' })
  },
  ...
  openMonthlyPerformance() {
    ...
    if (target && target !== '/pages/home/index') {
      if (CONTENT_TAB_TARGETS.has(target)) runtime.switchTab({ url: target })
      else runtime.navigateTo({ url: target })
      return
    }
    runtime.navigateTo({ url: '/pages/performances/index' })
  },
  ...
  openFeaturedActivity() {
    ...
    runtime.navigateTo({ url: `/pages/community-detail/index?id=${encodeURIComponent(activity.publicId)}` })
  },
  ...
  openEditorial(event) {
    ...
    if (!card) return
    if (card.isMboxStory) {
      runtime.navigateTo({ url: '/pages/brand-story/index' })
      return
    }
  ...
  remindMembershipInviteAgreement() {
    runtime.showToast({ title: '请先勾选同意协议', icon: 'none' })
  },
  ...
  async acceptMembershipInvite(event) {
    ...
    try {
      ...
      if (pendingActivityId) {
        runtime.navigateTo({ url: `/pages/community-detail/index?id=${encodeURIComponent(pendingActivityId)}` })
      }
    } catch (error) {
      this.setData({ error: membershipEnrollErrorMessage(error, '入会暂时没有完成') })
