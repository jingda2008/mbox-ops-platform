...
const CODE_MESSAGES = Object.freeze({
  ...
  TABLE_QR_INVALID: '新桌码无效，请扫描本店当前有效桌码',
  INVENTORY_RECIPE_MISSING: '所选商品供应配置尚未完成，请更换商品或联系服务人员',
  INVENTORY_BALANCE_MISSING: '所选商品库存资料待核对，请更换商品或联系服务人员',
  ORDER_NOT_PAYABLE: '所选订单当前无需付款或已结束，请刷新桌账核对',
  ...
  GUEST_CHECKOUT_CONFIGURATION_UNAVAILABLE: '暂时无法发起支付宝支付，本次没有创建订单，请联系服务员',
  ...
  WECHAT_IDENTITY_REQUIRED: '当前支付身份协议不适用于支付宝，本次没有扣款，请联系服务员',
  MEMBERSHIP_SERVICE_UNAVAILABLE: '会员服务暂时无法连接，请稍后重试',
  MENU_CATALOG_INCOMPLETE: '菜单没有完整加载，请检查网络后重试',
  MENU_CATALOG_RESPONSE_INVALID: '菜单数据暂时异常，请稍后重试',
  MENU_CATALOG_CHANGED: '菜单正在更新，请稍后重新加载',
  ...
  DAILY_SNACK_MEMBER_LIMIT_INVALID: '每日点心次数规则尚未配置完整，本次未新增申请，请联系服务人员',
  DAILY_SNACK_PRICE_INVALID: '每日点心商品价格未配置完整，本次未新增申请，请联系服务人员',
  ...
  ACTIVITY_PAYMENT_WECHAT_IDENTITY_REJECTED: '当前支付身份协议不适用于支付宝，本次没有扣款，请联系活动负责人',
  ...
  AUTHENTICATION_REQUIRED: '登录状态没有带上，请完全关掉支付宝小程序后重新打开，再授权入会',
  ALIPAY_PHONE_NOT_CONFIGURED: '支付宝已授权手机号，但现网尚未注入解密密钥，本次没有入会。浏览、预约和点单不受影响',
  ALIPAY_PHONE_AUTHORIZATION_INVALID: '支付宝手机号授权未能通过校验，本次没有入会。请再试一次或联系门店',
  ALIPAY_PHONE_PLATFORM_CONFIG: '支付宝开放平台仍缺接口内容加密、加签或应用网关。请补齐开发设置后，在小程序解除用户信息授权再重试',
  ALIPAY_PHONE_PLAINTEXT_REJECTED: '支付宝返回了未加密手机号，服务端已拒绝入会。请在开放平台确认接口内容加密后，再用真机重新授权',
  ALIPAY_PHONE_SESSION_INVALID: '会员会话无效，请完全关掉支付宝小程序后重新打开，再授权入会',
  WECHAT_PHONE_AUTHORIZATION_INVALID: '支付宝已授权手机号，但当前服务器仍按微信凭证校验，本次没有入会。浏览、预约和点单不受影响',
  MEMBERSHIP_ENROLLMENT_PHONE_NOT_CONFIGURED: '会员入会服务尚未接通支付宝手机号，本次没有入会。浏览、预约和点单不受影响',
  MEMBERSHIP_RECOVERY_PHONE_NOT_CONFIGURED: '会员手机号校验暂时不可用，请稍后重试或联系门店协助找回',
...
function customerErrorMessage(error, fallback) {
  ...
  return CODE_MESSAGES[code] || String(fallback || '服务暂时繁忙，请稍后重试').trim()
}

function membershipEnrollErrorMessage(error, fallback) {
  const code = customerErrorCode(error)
  if (code === 'ROUTE_NOT_FOUND') {
    return '会员登录服务暂时不可用，请稍后重试或联系门店'
  }
  if (code === 'PUBLIC_RESERVATION_REQUEST_INVALID') {
    return '支付宝已授权手机号，但现网还不接受该授权格式，本次没有入会。浏览、预约和点单不受影响'
  }
  return customerErrorMessage(error, fallback || '登录暂时没有完成')
}

function isAlipayCancellation(error) {
  // This is control flow only. The raw platform string is never displayed.
  if (error && error.code === 'ALIPAY_PAYMENT_CANCELLED') return true
  const raw = String((error && (error.errMsg || error.message)) || '').toLowerCase()
  return raw.includes('cancel')
}

export { customerErrorCode, customerErrorMessage, membershipEnrollErrorMessage, isAlipayCancellation }
