...
当前工程版本：`1.0.0-rc.177`。这是用于预发布和门店影子运行的商业V1候选版，不代表支付、微信、云环境、真实硬件和门店验收已经完成。
...
- 客户心情、七类服务、点单支付、吧台/厨房制作、取送、点歌和预约到店的跨岗位责任链路自动化验收；完成事项从客户待办自动消失，过程与未响应数据保留用于分析。
...
- 预约人数/时间修改、迟到保留决定、满台候补公平排序、通知锁桌、超时释放和候补入座。
- 营业中整桌换位与临时桌转正式桌；保持同一桌次、订单、支付、KDS、权益和点歌引用，旧桌码立即失效。
- 库存入库、盘点审批、调整流水、低库存阈值、客存酒登记/取用/转台/作废，以及员工端库存台账。
- 设备中台、心跳健康、场景绑定、权限隔离、模拟联调、关键事件前后窗口抽帧命令，以及摄像头、耳机、打印机、扫码器和边缘节点适配边界。
- PC、平板和手机响应式Web；顾客桌码样例。
- 全端点击即时反馈、按钮级异步提交和防重复触发；服务任务、KDS与预约高频动作采用乐观更新，服务端失败时自动恢复，支付、退款、库存和权限变更坚持服务端确认。
