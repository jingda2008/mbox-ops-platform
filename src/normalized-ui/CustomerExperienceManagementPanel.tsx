...
export function CustomerExperienceManagementPanel({ api, auth, dashboard, onChanged }: {
  ...
  // The consolidated operations workbench is now the only executable activity
  // authoring surface. This switch keeps the former form as inert history while
  // the rollout is reviewed, preventing two contracts from reaching operators.
  const legacyActivityEditorEnabled = false
  ...
  const [form, setForm] = useState({
    ...
    coverUrl: '', details: '', includedItems: '', participationRequirements: '', safetyRequirements: '',
    visibility: 'public', memberLevel: 'member', memberBenefitText: '', contactInstructions: '',
    refundPolicyVersion: 'activity-refund-v1', safetyPolicyVersion: 'activity-safety-v1',
  })
  ...
  async function createActivity(event: FormEvent) {
    ...
    try {
      ...
      if (form.visibility === 'segment' && form.memberLevel.trim() === '') throw new Error('指定会员等级活动必须填写会员等级')
      await api.postEndpoint('/api/staff/community-activities', {
        ...
        refundPolicySnapshot: {
          policyVersion: policyVersion(form.refundPolicyVersion, '退款规则版本'),
          summary: form.refundRule.trim(),
          capturedAt: new Date().toISOString(),
        },
        pointsReward: 0,
        visibility: form.visibility,
        audienceRule: form.visibility === 'segment' ? {
          memberLevels: form.memberLevel ? [form.memberLevel] : [],
        } : {},
        safetySnapshot: {
          policyVersion: policyVersion(form.safetyPolicyVersion, '安全规则版本'),
          requirements: lines(form.safetyRequirements),
          acknowledgementText: '我已阅读并同意本活动的安全与参与要求',
          capturedAt: new Date().toISOString(),
        },
        salesCopy: {
          details: form.details.trim(),
          includedItems: lines(form.includedItems),
          participationRequirements: lines(form.participationRequirements),
          memberBenefitText: form.memberBenefitText.trim(),
  ...
  return <div className="staff-module-body customer-experience-management">
    ...
    {legacyActivityEditorEnabled && showCreate && <form className="staff-module-form activity-config-form" onSubmit={(event) => void createActivity(event)}>
      ...
      <label className="activity-wide">列表摘要<input required minLength={2} maxLength={600} value={form.summary} onChange={(event) => set('summary', event.target.value)} /></label>
      <label className="activity-wide">活动详情<textarea required minLength={10} maxLength={4000} rows={5} value={form.details} onChange={(event) => set('details', event.target.value)} placeholder="说明活动内容、流程和体验亮点；客户点进详情后看到。" /></label>
      <label className="activity-wide">封面图片地址（可选）<input type="url" maxLength={1000} value={form.coverUrl} onChange={(event) => set('coverUrl', event.target.value)} placeholder="https://…" /></label>
      ...
      <label>可见范围<select value={form.visibility} onChange={(event) => set('visibility', event.target.value)}><option value="public">所有顾客</option><option value="member">仅会员</option><option value="segment">指定会员等级</option></select></label>
      <label>指定会员等级（可选）<select disabled={form.visibility !== 'segment'} value={form.memberLevel} onChange={(event) => set('memberLevel', event.target.value)}><option value="member">普通会员</option><option value="silver">银卡会员</option><option value="gold">金卡会员</option></select></label>
      <p className="activity-wide activity-payment-preview">活动积分奖励暂不开放：旧字段没有规则版本、预算和发放状态机，不会因报名或签到静默发分。</p>
      ...
      <label>退款规则版本<input required minLength={3} maxLength={64} value={form.refundPolicyVersion} onChange={(event) => set('refundPolicyVersion', event.target.value)} placeholder="例如 activity-refund-v2" /></label>
      ...
      <label className="activity-wide">费用包含内容<textarea required minLength={2} maxLength={1200} rows={3} value={form.includedItems} onChange={(event) => set('includedItems', event.target.value)} placeholder={'每行一项，例如：\n欢迎饮品 1 杯\n活动物料与摄影'} /></label>
      <label className="activity-wide">参与条件<textarea required minLength={2} maxLength={1200} rows={3} value={form.participationRequirements} onChange={(event) => set('participationRequirements', event.target.value)} placeholder={'每行一项，例如：\n须年满 18 周岁\n请提前 15 分钟到场'} /></label>
      <label>安全规则版本<input required minLength={3} maxLength={64} value={form.safetyPolicyVersion} onChange={(event) => set('safetyPolicyVersion', event.target.value)} placeholder="例如 activity-safety-v2" /></label>
      <label className="activity-wide">安全要求与风险提示<textarea required minLength={5} maxLength={1600} rows={4} value={form.safetyRequirements} onChange={(event) => set('safetyRequirements', event.target.value)} placeholder={'客户报名确认前必须看到并主动勾选；每行一项。'} /></label>
      <label className="activity-wide">会员权益或赠送内容<textarea maxLength={1200} rows={3} value={form.memberBenefitText} onChange={(event) => set('memberBenefitText', event.target.value)} placeholder="例如：报名成功赠送活动限定饮品券 1 张；没有则填写“无额外赠送”。" /></label>
      <label className="activity-wide">报名后联系与集合说明<textarea required minLength={2} maxLength={1200} rows={3} value={form.contactInstructions} onChange={(event) => set('contactInstructions', event.target.value)} placeholder="写明联系人、集合确认方式和临时变更通知方式。" /></label>
...
function policyVersion(value: string, label: string) { const normalized = value.trim(); if (!/^[A-Za-z0-9][A-Za-z0-9._-]{2,63}$/.test(normalized)) throw new Error(`${label}须为3至64位字母、数字、点、下划线或短横线`); return normalized }
