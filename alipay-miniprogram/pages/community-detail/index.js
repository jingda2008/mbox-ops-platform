...
const { customerErrorMessage, membershipEnrollErrorMessage, isAlipayCancellation } = require('../../utils/customer-error')
const {
...
} = require('../../utils/alipay-subscription')
const { rememberPresentationOptions } = require('../../utils/alipay-subscription-presentation-cache')
...
function pricingFor(activity, activityPackage, partySize) {
  ...
  const alipayAvailable = authorityAvailable
    && list(activity.availablePaymentMethods).some((method) => ['alipay_jsapi', 'trade_pay'].includes(method))
  ...
  return {
    ...
    paymentAvailability: alipayAvailable || paymentMode === 'none' || paymentMode === 'deposit_optional' ? 'available' : 'blocked',
    availablePaymentChoices,
    availablePaymentMethods: alipayAvailable
      ? list(activity.availablePaymentMethods).filter((method) => ['alipay_jsapi', 'trade_pay'].includes(method)) : [],
...
function viewRegistration(raw) {
  ...
  return Object.assign({}, raw, {
    ...
    paymentMethodText: ['alipay_jsapi', 'trade_pay'].includes(payment.method) ? '支付宝支付' : payment.method === 'native_qr' ? '二维码支付' : '',
    stateGuide,
  })
}
...
function alipayTradePayload(value) {
  if (!value || typeof value !== 'object') return null
  const tradeNO = String(value.tradeNO || '').trim()
...
Page({
  data: {
    ...
    // Keep the next action explicit. Identity recovery stays disabled until
    // the shared backend provides an Alipay identity adapter.
    alipayIdentityRefreshRequired: false,
    alipayNotificationPromptOptions: [],
    alipaySubscriptionPresentationOptions: [],
  },
  ...
  async load() {
    ...
    try {
      ...
      const [raw, activityPrompt, memberPrompt, couponPrompt] = await Promise.all([
        ...
        getAlipayNotificationPrompt('activity_registration').catch(() => ({ authorizations: [] })),
        getAlipayNotificationPrompt('member_card').catch(() => ({ authorizations: [] })),
        getAlipayNotificationPrompt('coupon_open').catch(() => ({ authorizations: [] })),
        this.preloadAlipaySubscriptionPresentationOptions(),
      ])
      const notificationPrompt = {
        authorizations: mergeAlipayNotificationPromptOptions(
      ...
      this.setData({
        ...
        alipayNotificationPromptOptions: notificationPrompt.authorizations || [],
        alipaySubscriptionPresentationOptions: notificationPrompt.presentation || [],
  ...
  dismissMembershipInvite() {
    ...
    runtime.navigateBack({ fail: () => runtime.switchTab({ url: '/pages/community/index' }) })
  },
  ...
  async acceptMembershipInvite(event) {
    ...
    try {
      ...
      this.setData({ membershipInviteBusy: false, error: membershipEnrollErrorMessage(error, '入会暂时没有完成') })
      runtime.showToast({ title: membershipEnrollErrorMessage(error, '入会未完成'), icon: 'none' })
    }
  },
  ...
  async preloadAlipaySubscriptionPresentationOptions() {
    try {
      const empty = { presentation: [], authorizations: [] }
      const [activityPrompt, memberPrompt, couponPrompt] = await Promise.all([
        getAlipayNotificationPrompt('activity_registration').catch(() => empty),
        getAlipayNotificationPrompt('member_card').catch(() => empty),
        getAlipayNotificationPrompt('coupon_open').catch(() => empty),
      ])
      ...
      if (options.length) this.setData({ alipaySubscriptionPresentationOptions: options })
      return options
    } catch (_error) {
      return this.data.alipaySubscriptionPresentationOptions || []
    }
  },

  async offerActivityRegistrationNotifications() {
    const options = this._presentationOptions || this.data.alipaySubscriptionPresentationOptions || []
    await requestAlipaySubscription(options, ACTIVITY_REGISTRATION_SUBSCRIBE_TYPES)
  ...
  async register() {
    ...
    if (pricing.requiresPaymentOnSubmit && getRuntimeConfig().alipayPaymentEnabled !== true) {
      return this.setData({ error: '支付宝在线支付后端适配尚未接通，本次报名尚未创建，也没有占用名额。' })
    }
    ...
    if (attempt && (attempt.payload.activityPackagePublicId || '') !== ((chosenPackage && chosenPackage.publicId) || '')) {
      this.clearRegistrationAttempt(activity.publicId)
      attempt = null
    }
    ...
    if (attempt && attempt.payload.contactSnapshot) {
      ...
      runtime.setStorageSync(REGISTRATION_ATTEMPTS_KEY, attempts)
    }
    let contact = ''
    if (attempt) {
      ...
      // the customer's original registration action.
      await this.offerActivityRegistrationNotifications()
      const payment = await this.choosePayment(pricing)
      if (!payment) return
      const payload = {
        activityPackagePublicId: chosenPackage && chosenPackage.publicId || null,
      ...
      attempt = {
        idempotencyKey: commandKey('activity-register', `${activity.publicId}-${chosenPackage && chosenPackage.publicId || 'ticket'}`),
    ...
    this.setData({ busy: true, error: '', success: '', alipayIdentityRefreshRequired: false })
    try {
      ...
      // immediately present the matching Alipay payment sheet instead of
      ...
      if (shouldAutoStartPayment) {
        this.setData({ success: '名额已暂留，正在打开支付宝支付。' })
      } else {
      ...
      if (error && error.code === 'WECHAT_IDENTITY_REQUIRED') {
        ...
        this.setData({
          alipayIdentityRefreshRequired: false,
          error: '当前后端支付身份协议不适用于支付宝，本次报名尚未创建，也没有占用名额；请联系活动负责人。',
        })
        return
      }
  ...
  async choosePayment(item) {
    const choices = list(item.availablePaymentChoices)
    const canUseAlipay = getRuntimeConfig().alipayPaymentEnabled === true
      && list(item.availablePaymentMethods).some((method) => ['alipay_jsapi', 'trade_pay'].includes(method))
    const multiplier = item.feeBasis === 'per_person' ? this.data.partySize : 1
    const options = []
    if (choices.includes('deposit') && canUseAlipay) options.push({ choice: 'deposit', method: 'alipay_jsapi', label: `支付宝支付订金 ${money(item.depositAmountMinor * multiplier)}` })
    if (choices.includes('full') && canUseAlipay) options.push({ choice: 'full', method: 'alipay_jsapi', label: `支付宝支付全款 ${money(item.feeAmountMinor * multiplier)}` })
    ...
    return new Promise((resolve) => runtime.showActionSheet({
  ...
  confirmPayment(content) {
    return new Promise((resolve) => runtime.showModal({ title: '确认本次报名', content, confirmText: '继续', success: (result) => resolve(result.confirm), fail: () => resolve(false) }))
  },
  ...
  showRegistrationOutcome(registration) {
    ...
    return new Promise((resolve) => runtime.showModal({
      ...
      cancelText: '留在本页',
      success: (result) => {
        if (result.confirm) runtime.switchTab({ url: '/pages/profile/index' })
        resolve()
      },
      fail: () => resolve(),
    }))
  ...
  async startPayment() {
    ...
    if (getRuntimeConfig().alipayPaymentEnabled !== true) {
      this.setData({
        error: '支付宝在线支付后端适配尚未接通，本页没有发起付款；请联系活动负责人。',
        alipayIdentityRefreshRequired: false,
      })
      return
    }
    ...
    runtime.setStorageSync(PAYMENT_ACTION_ATTEMPTS_KEY, stored)
    this.setData({ busy: true, error: '', success: '', alipayIdentityRefreshRequired: false })
    try {
      ...
      if (!['alipay_jsapi', 'trade_pay'].includes(action.presentation)) {
        this.setData({ error: '支付通道返回的不是支付宝小程序支付，本页不会用二维码或模拟结果替代。请联系活动负责人。' })
        return
      }
      const payload = alipayTradePayload(action.payload)
      if (!payload) {
        this.setData({ error: '支付宝交易号不完整，本页没有发起付款，也不会显示成功。' })
        return
      }
      try {
        await new Promise((resolve, reject) => runtime.requestPayment(Object.assign({}, payload, { success: resolve, fail: reject })))
      ...
      const message = preflightIdentityRequired
        ? '报名已保留，付款尚未发起。当前支付身份协议不适用于支付宝，请勿重复报名。'
        : rejectedIdentity
          ? '当前支付身份协议不适用于支付宝，本次没有扣款；请勿重复报名。'
          : customerErrorMessage(error, '支付动作结果暂时无法确认，请先查单，不要重复付款。')
      this.setData({
        alipayIdentityRefreshRequired: identityRefreshRequired,
        error: message,
      })
    } finally { this.setData({ busy: false }) }
  },

  async refreshAlipayIdentityAndRetry() {
    if (this.data.busy || !this.data.alipayIdentityRefreshRequired) return
    ...
    try {
      // A forced refresh only renews the anonymous reservation session. It
      // must not pretend an Alipay identity adapter exists.
      await ensureCustomerSession(true)
    } catch (error) {
      this.setData({ error: customerErrorMessage(error, '访客会话暂时无法刷新，请稍后再试。') })
      return
    } finally {
      this.setData({ busy: false })
    }
    this.setData({ alipayIdentityRefreshRequired: false })
  ...
  async performPaymentQuery() {
    ...
    stored[registration.publicId] = attempt
    runtime.setStorageSync(PAYMENT_QUERY_ATTEMPTS_KEY, stored)
    try {
  ...
  async cancelRegistration() {
    ...
    const confirmed = await new Promise((resolve) => runtime.showModal({ title: '取消报名', content: '仅未付款且仍可取消的报名会释放名额。确认取消吗？', confirmColor: '#873e30', success: (result) => resolve(result.confirm), fail: () => resolve(false) }))
    if (!confirmed) return
    ...
    stored[registration.publicId] = attempt
    runtime.setStorageSync(CANCELLATION_ATTEMPTS_KEY, stored)
    this.setData({ busy: true, error: '', success: '' })
    try {
  ...
  showCancellationHelp() {
    ...
    runtime.showModal({ title: '取消与退款说明', content: `已付款报名如需退款，请联系门店；退款进展会显示在这里。${contact}`, showCancel: false })
  },
  ...
  rememberRegistration(registration, activityPublicId) {
    ...
    runtime.setStorageSync(LOCAL_REGISTRATIONS_KEY, stored)
  },
})
