...
| 连接池 | 获取失败为0；等待P95不高于50ms、P99不高于100ms；结束时等待客户端为0 |
| 数据库事务 | 失败为0；P95不高于500ms、P99不高于1000ms |
| 领域SQL | 失败为0；P95不高于250ms、P99不高于500ms |
...
gate
  ...
  failures
serverMetrics
  database: 连接池、事务和领域SQL的有界聚合统计
  outbox: 待投递数量、最老消息年龄及本次是否启用真实投递验收
```
...
真实HTTP工具同时记录规范化事务层的连接池等待、事务耗时和领域SQL耗时，且不保存SQL正文、参数或连接字符串。隔离压测关闭外部集成worker，因此Outbox只记录积压和年龄，不把未投递计为性能失败；真实通知、支付、打印等Outbox投递仍必须在候选环境另行验收，不能由本测试冒充。
