        ...
        {draft !== null && <form className="catalog-management-form" onSubmit={(event) => void save(event)}>
          ...
          <div className="catalog-form-grid">
            ...
            {showAdvanced && <>
              ...
              <div className="catalog-wide">{auth.permissions.includes('media.asset.menu.manage') ? <MediaAssetPicker api={api} purpose="menu" value={draft.imageUrl} onChange={(imageUrl) => updateDraft('imageUrl', imageUrl)} label="上传或选择菜单图片" /> : <small>可选择上方已有菜单图片；上传新图片需商品图片管理权限。</small>}</div>
