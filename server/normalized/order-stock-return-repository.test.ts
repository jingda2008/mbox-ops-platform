import {legacyStockReturnCapabilitySql} from './order-stock-return-capability.js'
...
integration('partial physical returns',()=>{
 ...
 beforeAll(async()=>{
  ...
  await pool.query("INSERT INTO mbox.table_sessions(id,tenant_id,store_id,table_id,public_id,business_date,status,guest_count) VALUES($1,$2,$3,$4,'return-session','2026-08-01','open',2)",[session,tenant,store,table])
 ...
 const capability=()=>runner.run(scope,async tx=>{await tx.query('SET LOCAL ROLE mbox_runtime');return (await tx.query<{capability:Record<string,unknown>}>(`SELECT capability FROM mbox.order_items source CROSS JOIN LATERAL(${legacyStockReturnCapabilitySql('source')}) eligibility WHERE source.tenant_id=$1 AND source.store_id=$2 AND source.id=$3`,[tenant,store,item])).rows[0].capability})
 const historical=(includeStockReturnWork=true)=>runner.run(scope,tx=>readOperatingHistory(tx,{businessDate:'2026-09-13',earliestBusinessDate:'2026-09-10',table:'',employee:'',page:0,includeStockReturnWork,allowFinancialSummary:false}))
 it('does not restore inventory merely from a request or unconfirmed physical state',async()=>{
  expect(await capability()).toMatchObject({canReturnUnmade:false,canReturnUnopened:false,remainingQuantity:4})
  expect((await historical()).orders).toHaveLength(0)
  await expect(record(2)).rejects.toThrow('尚无确认成功的本商品')
 ...
 it('records two partial returns and rejects concurrent excess without issuing another refund',async()=>{
  ...
  await pool.query(`INSERT INTO mbox.refunds(id,tenant_id,store_id,payment_id,public_id,provider_refund_id,amount_minor,currency,status,reason,requested_by_employee_id,approved_by_employee_id,decision_reason,completed_at)
   VALUES($1,$2,$3,$4,'return-refund','offline-return-voucher',4000,'CNY','succeeded','顾客退货',$5,$6,'实际现金退付',clock_timestamp())`,[refundId,tenant,store,payment,employee,reviewer])
  await expect(record(1)).rejects.toThrow('尚无确认成功的本商品')
  await pool.query("INSERT INTO mbox.refund_items(tenant_id,store_id,refund_id,order_item_id,amount_minor,currency) VALUES($1,$2,$3,$4,4000,'CNY')",[tenant,store,refundId,item])
  expect(await capability()).toMatchObject({canReturnUnmade:false,canReturnUnopened:true,remainingQuantity:4})
  const overdue=await historical()
  expect(overdue.orders.map(row=>row.id)).toEqual([order])
  expect(overdue.orders[0].items[0].stockReturn).toMatchObject({canReturnUnopened:true,remainingQuantity:4})
  expect((await historical(false)).orders).toHaveLength(0)
  await pool.query("UPDATE mbox.inventory_items SET base_unit='ml',package_volume_ml=700 WHERE id=$1",[inventory])
  expect(await capability()).toMatchObject({canReturnUnmade:false,canReturnUnopened:false})
  expect((await historical()).orders).toHaveLength(0)
  await pool.query("UPDATE mbox.inventory_items SET base_unit='bottle',package_volume_ml=NULL WHERE id=$1",[inventory])
  await expect(record(1,{disposition:'unmade'})).rejects.toThrow('未制作退库须先停止出品')
  await expect(record(5)).rejects.toThrow('累计退回数量超过')
  await record(2)
  expect(await capability()).toMatchObject({canReturnUnopened:true,remainingQuantity:2})
  ...
  expect(await capability()).toMatchObject({canReturnUnmade:false,canReturnUnopened:false,remainingQuantity:0})
  expect((await historical()).orders).toHaveLength(0)
 })
})
