...
describe('member content card mini-program contract', () => {
  it('keeps content recommendations separate from issued benefits and below core member information', () => {
    ...
    expect(view).toContain('活动与内容推荐')
    expect(view).toContain('仅供了解，不代表权益已到账')
    expect(view).toContain('已到账的赠送和权益只在上方“我的权益与赠送”中显示。')
    expect(view.indexOf('我的权益与赠送')).toBeLessThan(view.indexOf('活动与内容推荐'))
    expect(view.indexOf('接下来的安排')).toBeLessThan(view.indexOf('活动与内容推荐'))
    expect(view.indexOf('最近积分')).toBeLessThan(view.indexOf('活动与内容推荐'))
  })
  ...
  it.each([320, 390])('uses a compact horizontal card rail at %ipx without reducing action targets', (viewportWidth) => {
    ...
    const cardWidthRpx = viewportWidth <= 350 ? 520 : 500
    ...
    expect(css).toMatch(/\.member-content-copy button \{[^}]*min-height: 44px;/s)
