...
test('one business-day order and guest requests flow through bartender, kitchen, service and manager roles', async ({ browser }) => {
  ...
  await expect(cashier.page.getByText('1 笔待确认支付 · 0 笔已确认流水')).toBeVisible()
  await expect(cashier.page.getByText(/尚未入账/)).toBeVisible()
  await expect(cashier.page.locator('.staff-module-list article')).toHaveCount(0)
