...
test('mobile manager payment choices stay synchronized with two guests at the same table', async ({ page, browser }) => {
  ...
  await page.getByRole('button', { name: '现场', exact: true }).first().click()
  await expect(page.getByRole('heading', { name: '找到桌台，直接处理' })).toBeVisible()
  await page.getByRole('button', { name: /^W1 \d+人 · / }).click()
  const tableActions = page.getByRole('dialog', { name: 'W1桌台操作' })
  ...
  const memberBenefitTasks = tableActions.getByRole('region', { name: 'W1会员权益' })
  await expect(memberBenefitTasks).toBeVisible()
  await expect(memberBenefitTasks).toContainText('当前范围没有会员权益暂留。')

  await tableActions.getByRole('button', { name: '协助点单' }).click()
  const paidSheet = page.getByRole('dialog', { name: 'W1协助点单' })
  ...
  await expect(page.getByRole('status')).toContainText('W1 订单已挂桌并发送出品')

  await tableActions.getByRole('button', { name: '本桌收款' }).click()
  const tablePayment = page.getByRole('dialog', { name: 'W1本桌收款' })
  ...
  const cashConfirmation = page.getByRole('alertdialog', { name: '确认 W1 已收到现金' })
  ...
  await tableActions.getByRole('button', { name: '协助点单' }).click()
  const immediateSheet = page.getByRole('dialog', { name: 'W1协助点单' })
  ...
  await tableActions.getByRole('button', { name: '本桌收款' }).click()
  const reopenedTablePayment = page.getByRole('dialog', { name: 'W1本桌收款' })
  ...
  await tableActions.getByRole('button', { name: '协助点单' }).click()
  const barcodeSheet = page.getByRole('dialog', { name: 'W1协助点单' })
  ...
  const giftSheet = page.getByRole('dialog', { name: 'W1赠送商品' })
  ...
  await expect(page.getByRole('status')).toContainText('W1 商品已赠送并发送出品')

  await page.goto(data.guestUrl)
  await expect(page.getByTestId('normalized-guest-app')).toBeVisible()
  await page.getByRole('button', { name: /历史已下单.*W1/ }).click()
  ...
  await secondGuest.getByRole('button', { name: /历史已下单.*W1/ }).click()
