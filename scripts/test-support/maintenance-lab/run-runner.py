...
initial='5cd9f2993e845a2049484a5e1eab6d5b67718d26';final='34360ceb974e9d15475ed9523b2e5b4c2f43bbbd';old='5b9d929499b1d8cb0eb3a0c0668604e9a398f1fe'
...
def fetch(version,commit):
 ...
 return dest,m
old_dir,old_manifest=fetch('1.0.0-rc.216',old);final_dir,final_manifest=fetch('1.0.0-rc.219',final)
run(['docker','tag',old_manifest['imageTag'],'audit-maintenance-source-live:5b9d929']);run(['docker','tag',final_manifest['imageTag'],'audit-maintenance-final:34360ce'])
assets=root/'assets';assets.mkdir();shutil.copytree(fixture,assets/'fixture');shutil.copytree(old_dir,assets/'old-release')
unpack_source(initial,assets/'source-5cd9f29');unpack_source(final,assets/'source-34360ce')
metadata=root/'initial-build.json';initial_tag='audit-maintenance-initial:5cd9f29'
run(['docker','buildx','build','--load','--build-arg','APP_COMMIT_SHA='+initial,'--build-arg','APP_RELEASE_VERSION=1.0.0-rc.219','--metadata-file',metadata,'-t',initial_tag,'.'],cwd=assets/'source-5cd9f29')
...
images={initial:{'tag':initial_tag,'digest':exported_digest,'config':config,'archive':'initial-image.tar'},final:{'tag':'audit-maintenance-final:34360ce','digest':final_manifest['imageDigest'],'config':final_manifest['platformImageDigest'],'archive':'final-image.tar.gz'}};(assets/'images.json').write_text(json.dumps(images));(artifacts/'input-identities.json').write_text(json.dumps({'source':old_manifest['releaseSha'],'images':images,'nativeArchitecture':os.uname().machine,'productionCredentialsUsed':False},indent=2)+'\n')
run(['docker','build','-t','audit-maintenance-full-entry:rc219',fixture]);run(['docker','pull','postgres:16-alpine']);run(['docker','pull','caddy:2.10.2-alpine'])
inner_images=root/'inner-images.tar';run(['docker','save','-o',inner_images,'audit-maintenance-source-live:5b9d929',initial_tag,'audit-maintenance-final:34360ce','caddy:2.10.2-alpine'])
for scenario in ['success','forward']:
 ...
 try:
  cid=text(['docker','run','-d','--name',host,'--hostname',host,'--privileged','--cgroupns=private','--network','none','--memory','4g','--memory-swap','4g','--cpus','3','--pids-limit','1024','--shm-size','256m','--tmpfs','/run','--tmpfs','/run/lock','--tmpfs','/tmp','audit-maintenance-full-entry:rc219']);created.append(cid)
