...
type NotificationType = WechatLoyaltyNotificationType | WechatMemberServiceNotificationType
...
export function decideWechatNotificationPrompt(input: Readonly<{
  ...
  const candidates: WechatNotificationPromptOption[] = [
    ...
    item.templateId.trim().length > 0
    && item.usesRemaining <= 0
    && item.platformResult !== 'ban'
  ))
