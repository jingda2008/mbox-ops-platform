...
function ReservationRow({ reservation, focused, salesName, access, busyAction, onQuickAction, onOpenOperation, onSaveCard }: {
  ...
  return <article id={`reservation-${reservation.id}`} className={`reservation-row status-${reservation.status}${focused ? ' is-ai-focus' : ''}`}>
    ...
    <div className="reservation-placement"><span><MapPin size={13} />{reservation.tableCode ?? reservation.requestedTableCode ?? reservation.areaPreferenceCode ?? '区域待定'}</span><small><UserPlus size={12} />{salesName}</small>{reservation.assignmentMode === 'self_select' && reservation.requestedTableCode && <b>客人自选</b>}{reservation.occasionCode && <b>{occasionLabel(reservation.occasionCode)}</b>}</div>
