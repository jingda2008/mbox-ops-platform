...
class Host:
    ...
    def preflight(self):
        ...
        for path in (self.root/'maintenance').glob('*/journal.jsonl'):
            ...
            require(records and records[-1]['event']=='completed','another maintenance transition remains unfinished')
        ...
        for kind,path in [('store',self.store),('catalog',self.catalog)]: require(sha(path)==self.manifest['configuration'][kind]['sha256'],'config binding mismatch')
    ...
    def backup(self):
        ...
        env={**self.pg_env,'DATABASE_SERVICE':self.backupservice,'MBOX_EXPECTED_RESTORE_DATABASE':self.database}
