...
test('configuration and migration checks precede every database write and application candidate', async () => {
  ...
  const previousReady = activate.indexOf('fetch_public_ready_response 200')
  const maintenanceReady = activate.indexOf('fetch_public_ready_response 503')
  assert.ok(publicRetry > 0 && publicRetry < previousReady && previousReady < migrationCompatibility)
  assert.ok(maintenanceReady > migrationCompatibility && maintenanceReady < writerDrain)
  assert.match(activate.slice(publicRetry, previousReady), /connect-timeout 3[\s\S]*max-time 8[\s\S]*sleep 2/)
