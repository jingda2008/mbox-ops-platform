...
## 1.0.0-rc.241 - 2026-09-26

- 收银复核只读订单查询按已选付款事实逐笔索引读取历史付款，避免数据库在历史关桌核对时反复扫描付款表。
- 修复手机收银页原订单权益与归属恢复卡片被压成窄列；保留待收、退款、已关桌补收及权限判断。schema保持250。

## 1.0.0-rc.240 - 2026-09-26

- 员工扫描会员码查询活动报名与可用券，选择仅到店签到或进入原活动签到。
- 新增独立到店记录、营业日去重与撤回；可配置签到次数和赠品，每累计一轮独立申请，管理人员单独或按日批量审批后发券。
- 审批与赠券同事务，预算不足整批回滚；保留原签到、已发权益和核销记录，防止重复领奖。schema249–250仅新增签到/奖励表及保护函数，不自动启用活动或发券。

## 1.0.0-rc.239 - 2026-09-26

- 微信及支付宝清空购物车确认固定原桌次和版本，确认期间换桌或购物车更新不再误清新状态。
- 取餐受限账号的旧出品异常提供明确经理交接入口，有权限账号可到达原任务；保持原回执恢复与最小权限。
- 补充两端22项确认竞态回归及真实登录浏览器交接验证；schema保持248。

## 1.0.0-rc.238 - 2026-09-26

- 订单中心、收银退款与桌台相关搜索优先匹配完整桌号，避免订单/付款编号片段或相邻编号混入。
- 现场、责任桌、后厨吧台、制作取送和会员权益复用一致的桌号匹配，补齐历史和经营分析旧编号兼容。
- 保留真实桌台身份、责任权限、取餐快照和撤回规则；schema保持248，无业务数据迁移。

## 1.0.0-rc.237 - 2026-09-25

- 按确认名单配置68桌，新增普通桌4人、VIP6人；保留未结束旧桌次。
- 桌号改名保留原ID及二维码，旧网页二维码仅兼容明确的28组旧新编号；不同桌仍拒绝。
- 配置同步检测新旧编号冲突并整笔回滚，避免重复建桌；schema保持248。

## 1.0.0-rc.236 - 2026-09-25

- 修复发布轮换门店口令后，已签发且仍有效的员工会话可登录却无法制作/取餐的问题；统一六小时会话规则。
- 保留员工、会话、设备、在线状态和岗位权限检查；新登录仍验证当前门店口令。
- schema248同步取餐凭据触发器与API校验，仅更新函数定义，不改订单、库存或历史授权记录。

## 1.0.0-rc.235 - 2026-09-24

- 旧出品页遇到制作屏批次时返回明确恢复提示，保留原数量、库存和幂等保护。
- 后厨/酒水刷新检查页面版本，有更新时保存当前份数并重载；失败保留队列，未决操作先恢复。
- 竖屏首次读取只有在制批次时显示制作中，后续轮询不改变员工选择；schema保持247。

## 1.0.0-rc.234 - 2026-09-24

- 修复后厨/酒水默认入口、旧页完成新制作批次与最小权限取餐登录循环；不扩大岗位权限。
- 三屏压缩重复顶栏，后厨横屏双栏、竖屏工序切换；保留备注、份数和原操作恢复。
- 适配键盘可见视口，后台/离线先锁定，恢复后权威重读。补充独立三屏CI回归；schema保持247，无新增数据迁移。
...
- 微信入会、找回和更换手机号声明 `phoneAuthorizationProvider=wechat`；资料页引导失败时仍读取已发布会员条款，避免误报协议未发布。
- 支付宝隐私读取改为与微信相同的匿名请求。不关闭 SYS-201，不代员工发布，本候选不等于已部署。

## 1.0.0-rc.231 - 2026-09-24

- 在线员工权限使用数据库微秒时间，修复刚撤销交接权限仍短暂有效；历史时间查询保持原语义。
- 历史商品记录和导出明确区分“有出品记录、送达未登记、桌次跨日结束”；原收付款、库存和履约事实不变。
- 三屏入口提示尚未配置实际取餐设备，功能开关不再被当成设备设置完成的证据；schema仍245，无数据迁移。

## 1.0.0-rc.230 - 2026-09-24

- 修复会员审批、叠加规则、优惠报价及预约政策的受限账号行锁问题，保留不可变历史和并发顺序。
- 通过限定函数提供通知管理、仅草稿子项替换和赠品商品锁，不授予营业账号直接更新政策/商品映射权限；schema244/245。
- 增补真实受限LOGIN七域配置、越权和并发验收；不修改正确收退款账目。

## 1.0.0-rc.229 - 2026-09-24

- 修复通知授权、预约变更确认、转桌、经营记录更正及活动积分退款的受限运行角色行锁错误，保留并发、身份、审计及不可变事实保护。
- 增补真实受限LOGIN回归；发布前验证公网TLS、远端Bash及镜像默认用户读取公开配置的能力。schema243仅把通知身份校验函数改用等价内建SHA256，限制锁等待，不扩大权限。
- 后续配置审批/优惠及预约数据库专项问题和实物验收缺口单独保留，不能据此声称全部经营路径已验收。

## 1.0.0-rc.228 - 2026-09-23

- 包含微信登录、付款身份查询修复；普通发布只读重验已完成维护计划覆盖的历史撤回记录，避免已结束的历史维护永久阻断后续发布。
- 保留全部日志、撤回凭据和写入边界；哈希、计划、当前镜像、schema、守护进程或回调状态不符时继续拒绝。

## 1.0.0-rc.227 - 2026-09-23

- 延续rc.226微信身份权限修复；付款与通知身份映射使用PostgreSQL内建SHA256，避免依赖pgcrypto所在schema及search_path。
- 受限账号回归固定pg_catalog查找路径，覆盖新老客登录、付款与通知身份解析及越权拒绝；无迁移。

## 1.0.0-rc.226 - 2026-09-23

- 修复微信登录身份保存错误更新受保护主体字段，导致认证503、购物车下单409的问题；保留原身份绑定与数据库权限。
- 新增受限PostgreSQL完整登录、付款身份关联及绑定冲突回归，无数据库迁移，无需更新小程序包。

## 1.0.0-rc.225 - 2026-09-23

- 重复收款请求在锁单后复用相同分摊、金额、渠道和方式的未释放尝试，避免新请求键制造重复支付记录。
- 对明确审核的历史系统尝试追加原订单与收退款关联，停止无效重试并保留真实晚到回调；发布只接受固定逐笔证据，不改原资金基线或正确账目。

## 1.0.0-rc.224 - 2026-09-23

- 修复旧未迁移撤回记录在后续已迁移维护重入时误拒绝schema的缺口；仅接受当前哈希日志中已实际provisioned的绑定目标。
- 财务、吧台、三屏业务及schema242不变；保留原资金、备份与向前恢复条件。

## 1.0.0-rc.223 - 2026-09-23

修复管理员所有的隔离恢复库无法按非超级用户原属主创建可信扩展的问题：仅在同一事务内临时授予并收回恢复库CREATE权限，保留扩展属主及源库ACL。业务与schema242保持rc.222，原标签及失败记录保留；正式升级验收另记。

## 1.0.0-rc.222 - 2026-09-23

恢复财务欠款/补收/奖励处理与吧台制作、三屏取走流程，以及225–242数据库迁移。修复RDS容器客户端临时文件不可见、恢复验证触及源库和未迁移维护撤回接续；保留rc.221兼容修复。最终生产验证另记。

## 1.0.0-rc.221 - 2026-09-23

- 从生产rc.216/schema224整理兼容修复：扫码并发、支付与退款锁顺序、商户绑定、撤权重放及后台任务健康状态。
- 使用原发布程序，不引入数据库迁移或账号切换；rc.217–220的升级内容暂缓，历史记录保留。
- 验证与暂缓范围见docs/release-1.0.0-rc.221.md；尚未部署。

## 1.0.0-rc.220 - 2026-09-21

- 发布传输不再继承发布端UID/GID；重试时将精确维护计划恢复为root:0600，保留正式入口的权限校验。
- 保持rc.219业务代码和schema242；保留首次生产预激活失败及其旧线上未切换的证据。

## 1.0.0-rc.219 - 2026-09-21

- 修复systemd219把SSH临时会话混入持久服务清单导致维护计划误判的问题，持久服务变化仍严格阻断。
- 新增242迁移，将关桌保护definer与新私有财务校验统一至验证过的共同维护owner，保持函数正文和原财务规则，撤公共及runtime直接执行权。
- 增加跨迁移账号升级及实际受限LOGIN完整财务回归；正式产物、原生维护和生产验收仍须分别通过。

## 1.0.0-rc.218 - 2026-09-21

- 修复跨迁移账号升级后，旧门店权限初始化函数无法推进新权限版本的问题。
- 新增241前向迁移，固定触发器查询路径、限定权威表并撤销直接执行权；保持旧登录禁用、原函数归属和001—240校验和不变。
- 增加真实数据库跨账号升级、跨门店及伪造触发来源回归；保留rc.217原生演练失败证据，生产切换仍须完成维护及恢复验收。

## 1.0.0-rc.217 - 2026-09-21

- 整合吧台、后厨、取走三屏与交接恢复，保留原份数、库存及幂等回执。
- 统一退品后消费待收、关桌阻断与日报投影；补齐实际商品退款积分成长的独立财务复核。
- 修复顾客服务原请求恢复、完整桌账、投诉选择与优先队列，以及扫码和收款履约并发。
- 普通重收保留已退服务补偿，持续欠款不随付款许可失效；首次奖励、履约及推荐按实际消费结清处理。
- 历史已关桌欠款通过受限授权补收，原桌次保持关闭；未外送本地付款须单独确认作废，未知渠道结果先核对。
- 普通退款实际补收后追加商品、推荐及会员贡献恢复事实；历史缺口经订单预览和异人复核处理，积分抵欠来源不明的分支保持待核。
- 商品金额采用原下单期累计退款及补收的统一口径；历史本地作废按钮与接口现有权限对齐。
- 修复系统外实收凭证字段被内部清洗误删，保留普通、批量和历史补收的原权限及单次入账保护。
- 候选数据库升级至schema238，运行权限与迁移管理分离；首次切换采用计划维护，失败保持维护并向前修复。
- 保留PrintBridge备注放大与双端小程序变更，平台上传、Windows安装和现场验收独立记录。

## 1.0.0-rc.216 - 2026-09-21

- 包含rc.215员工后台修复和后厨合批功能；生产代码和schema 224保持不变。
- 后厨真实数据库测试的员工会话使用同一个语句时间基准，满足精确6小时会话约束，消除多次读取墙上时钟造成的间歇初始化失败。
- 保留未部署的rc.215标签及失败证据；由新的主线提交和rc.216标签重新执行完整发布检查。

## 1.0.0-rc.215 - 2026-09-21

- 整合员工后台17项修复、60项文案处置及后厨跨订单合批工作台；沿用主线员工操作恢复与工作台体验修复。
- 新增迁移223损耗独立复核与224后厨持久批次；实际份数、库存、原桌次、设备占用和重复回执在事务中核对。
- 修复审查发现的混合旧流程问题：直接备齐仅处理明确选中的待制作份数，保留旧流程已开做份数。
- 修复盘点与损耗复核组件标识冲突造成刷新后重复盘点面板，保留独立审核、过期拒绝和原凭据重试。
- 后厨工作台由独立开关控制；发布证据与门店真机、实物取送和实体票据验收分别记录。

## 1.0.0-rc.214 - 2026-09-20

- 包含 rc.213 的四平台团购核销（大众点评、美团、抖音、快手）及 schema 222 / 门店配置 2026.09.20-v25。
- 删除未使用的 `providerVerifyId` 局部变量，修复标签 CI 中 `tsc` TS6133 导致 quality 与浏览器 webServer 无法启动。
- 本版不部署、不写入真实平台密钥，不把编译修复或模拟核销当作现场验收；SYS-268 保持开放。

## 1.0.0-rc.213 - 2026-09-20

- 收银与退款支持大众点评、美团、抖音、快手团购券查询与核销；先查券再确认消耗，本地保留核销与尝试记录。
- 凭证只从受保护环境变量读取；`MBOX_VOUCHER_MODE` 支持 disabled/test/uat/production，缺一组平台配置则该平台显示未配置，不误核销。
- schema 222 为核销记录增加平台码与平台回执，并新增只追加尝试流水；OWNER/MANAGER/CASHIER 授予 `commercial.voucher.view/redeem`。
- 门店配置更新为 `2026.09.20-v25` 同步权限。本版不部署、不写入真实平台密钥，也不把模拟核销当作生产验收。

## 1.0.0-rc.212 - 2026-09-20

- 营业日自动开日调整为11:00，结束仍为次日06:00；06:00—11:00处理上一日结转但不自动开启当天营业日。
- 员工工作台显示营业时间，预约时间及预约逻辑保持原样。
- 不改资金营业日归属、历史账目、手工提前结束营业日和未结事项保护；无数据库迁移。

## 1.0.0-rc.211 - 2026-09-19

- 后厨刷新失败保留队列、慢请求合并轮询、制作确认后锁定旧份数直到权威读回；原编号恢复按员工隔离。
- 本人制作/送达历史刷新并显示本人实际份数和时间，修复通知跳页、取消旧快照覆盖和配送合单提前解锁。
- 微信会员二维码支持点击放大、关闭按钮与遮罩关闭；支付宝不改。修正微信布局测试的既有页面数断言，保留25页逐页校验。
- 无数据库迁移，schema保持221；现场弱网、高峰及旧版无归属待确认操作仍需实际核验。

## 1.0.0-rc.210 - 2026-09-19

- 修复通用支付回收幂等键超长、失败健康状态漏报及渠道异常诊断丢失，保持渠道核验和单次账本约束。
- 接通服务号验签事件持久化，订阅成功只登记授权，不再发送示例消耗额度；修正 bizsend 受理及验证码有效期。
- 失效通知会话返回认证错误；员工图库按用途分页、缩略图和权限校验后的私有缓存；修复推荐只读锁及关闭状态重复请求。
- schema 221 新增只追加订阅上报记录及图库分页索引；发布前置浏览器预检，恢复完整中文验收清单并增加防截断门禁。

## 1.0.0-rc.209 - 2026-09-19

- 结账票按已确认收款事实区分预结账与结账，增加完整应付合计及实收、退款、待收分列。
- 统一结账/支付票据商场抬头和说明，提供已确认r4样式的PrintBridge 1.0.10可复现升级包。
- 保留原快照补打、退款及合付分摊语义；G1/G2为已生效临时桌位，无新增数据库迁移。

## 1.0.0-rc.208 - 2026-09-17

- 修复员工端服务号账号保存后无法展开编辑表单。
- 群发改为可选服务号、分日期/时间，并增加「保存并安排发送」。

## 1.0.0-rc.207 - 2026-09-17

- 将旧热修复替换记录的操作决定字段与认证凭据字段分离，保留完整发布证据的凭据扫描，补充真实扫描回归。
- 包含rc.206全部我的存酒功能；rc.206未部署，以rc.207继续交付。

## 1.0.0-rc.206 - 2026-09-17

- 微信个人页新增我的存酒：本人完整单据、历史取存、完整手机号和照片预览，沿用品牌绿。
- 发布支持精确绑定旧容器和两份镜像身份的获授权热修复替换，保留原清单与旧容器回滚。

## 1.0.0-rc.205 - 2026-09-16

- 新增可配置会员编号、会员存取酒及服务号验证码、余量/照片水印/联系电话、报表筛选与导出。
- 新增服务号/企微资格与会员卡菜单价格、活动赠品不叠加规则、可配置推荐弹窗。
- 修正菜单详情关闭和套餐长内容图片布局；推荐海报展示商品图片价格并支持最多8张左右滑动。
- IP风控增加可信客户端IP和明确拒绝分类；历史事故根因及真实平台、真机和实物验收继续开放。

## 1.0.0-rc.204 - 2026-09-16

- 按星驿正式协议识别共用回调地址的手续费通知，验签后独立持久化并幂等确认，不改变收退款状态。
- 共用入口分流支付、退款成功/失败与退款处理中通知；字段/类型错误与签名、商户错误分别记录。
- schema 204 新增按租户门店隔离的只追加手续费回执表，保存必要金额和证据摘要，不保存原始签名或顾客信息。

## 1.0.0-rc.203 - 2026-09-15

- 新增统一退款复核岗位权限/单次额度配置及人员缺项提示，无效授权整批回滚，保留禁止自审和金额上限。
- 集成结账单名称、陆家嘴中心抬头、手动整桌票新编号及连续补打追溯；rc.201/202未部署。
- 不自动赋予员工权限、调整真实额度或处理退款；schema203不变。

## 1.0.0-rc.202 - 2026-09-15

- 整桌手动结账单采用年月日-时分秒-六位数字编号；新请求新号，同票重试、分页及补打保持原号，尾部保留原始桌次追溯码。
- 通过现有打印桥的单号字段兼容1.0.9，无需更新门店程序；包含rc.201名称和商场抬头，rc.201未部署。

## 1.0.0-rc.201 - 2026-09-15

- 整桌消费账单改名为结账单，加入陆家嘴中心 L+MALL，打印入口同步名称。
- 新票保留真实收款、退款和原订单金额；旧票补打及其他票种保留原快照语义。
- 兼容门店现有打印桥的标题/副标题字段，无需替换 Windows 打印和切纸程序；schema203不变。

## 1.0.0-rc.200 - 2026-09-14

- 盘点复核支持当前页勾选、批量通过和统一原因批量退回，逐单展示成功与未完成结果。
- 自审、过期盘点和后台权限/库存变动保护继续有效；网络未确认重试沿用原单幂等请求。
- 原单审核成功后汇总刷新失败单独提示，避免将已完成审核显示成业务失败；schema203不变。

## 1.0.0-rc.199 - 2026-09-14

- 补齐商品售后受控审批拒绝日志的门店作用域、命令与处理阶段，验证日志字段且不输出敏感载荷。
- 收银静默刷新失败保留原列表并明确标记更新延迟，避免将旧状态误作新结果。
- 包含rc.198全部售后修复及schema203；rc.198未部署，本版作为正式发布候选。

## 1.0.0-rc.198 - 2026-09-14

- 商品售后退款从收银进入原售后单，旧普通审批接口返回受控冲突；保留独立审核、额度和原资金幂等保护。
- 商品处理弹窗固定关闭栏、独立正文滚动，适应可见视口，关闭和重开保留原操作恢复；换品子层可独立退出。
- 新库存预留、销售扣减和重做记录保存不可变包装证据；退库预检与执行使用同一依据，规格冲突明确转库存核对，整瓶配置保存校验。
- 待办分列资金、实物、岗位通知，显示通知时间。历史 ¥78 退款不重复操作，实际包装核对和 iPhone 微信实机验收保留。
- 包含最新主线结账单稳定识别号；Windows 打印桥候选安装与实物票验证另行执行。新增规范化迁移203。

## 1.0.0-rc.197 - 2026-09-14

- 员工制作队列在在线资格过期时先续期并重新核验，保留真实失效登录和操作权限保护。
- 退回盘点可逐项重新实测并提交新单，清空旧数量，保留禁止自审、库存变化保护和历史记录。
- 补充手机恢复、退回重盘及窄屏布局验证；仅更新员工网页，无新增数据库迁移。

## 1.0.0-rc.196 - 2026-09-14

- 员工“我的已制作”历史按订单和商品纵向展示，修复手机多商品与长单号横向溢出，保留商品处理入口。
- 库存页增加盘点复核、通过、退回和已处理查询；有审批权限的同事查看门店待办，提交人查看本人结果。
- 沿用禁止自审、门店隔离、失效盘点保护和可恢复请求；盘点后库存变动提示重新清点，不更改既有审批授权。

## 1.0.0-rc.195 - 2026-09-14

- 新增微信服务号安全模式回调校验路由，验证签名、时间窗、AES-CBC/PKCS#7密文和服务号AppID。
- 新增三项可选服务号回调配置及部署保留名单；未配置时现有业务行为不变。
- 入站事件持久消费者尚未接通时POST明确失败关闭，不伪装已接收；本版仅用于打通微信后台URL验证。

## 1.0.0-rc.194 - 2026-09-14

- 微信加菜不再等待慢桌账，提交前仍核验付款锁；协议匿名读取、重试及独立饮食信息授权。
- 修复非法JSON错误分类、真实政策日期读取和数据库营业日测试。
- 员工登录增加设备之外的账号、来源、门店保护；成功仅释放本次计数，消除嵌套限流事务的连接池等待。
- 修复扩展营业回归夹具金额/页面前置条件、售后触控高度，Vitest升级4.1.11。
- schema202与数量售后开关保持；微信上传和正式平台/真机验收分别记录，支付宝不发布。

## 1.0.0-rc.193 - 2026-09-14

- 修复发布环境配置规范化时遗漏数量售后开关：显式开启/关闭均原样保留，缺省仍关闭，错误值仍交由运行配置校验拒绝。
- 增加两种环境下四种开关状态、连续规范化的回归；业务代码、数据库schema202、门店v24及小程序内容沿用rc192。
