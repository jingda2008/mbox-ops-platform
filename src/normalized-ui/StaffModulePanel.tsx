...
import { InventoryStockCountPanel } from './InventoryStockCountPanel'
...
export function StaffModulePanel({ api, auth, module, initialBlockerFact = null, onLoginRequired, onNavigate }: {
    ...
    try {
      if (module === 'payments' || module === 'orders') {
        ...
        const [devices, jobs, bridges, routes] = await Promise.all([
          ...
          canViewPrintJobs ? api.getEndpoint<{ data: unknown }>(new URLSearchParams(window.location.search).has('printJobId') ? '/api/hardware/print-jobs?status=failed,dead&limit=100' : `/api/hardware/print-jobs?status=${auth.permissions.includes('print.reprint') ? 'pending,printing,printed,failed,dead' : 'pending,printing,failed,dead'}&limit=50`) : Promise.resolve({ data: [] }),
  ...
  return <section className="staff-module-panel" aria-label={title} data-action-reveal>
    <header>
      ...
      <div><small>工作台</small><h1>{title}</h1></div>
      <button
        type="button"
        aria-label={`刷新${title}`}
    ...
    {initialBlockerFact !== null && <aside className="staff-module-blocker-fact" data-blocker-fact-id={initialBlockerFact.id}>
      <small>上一营业日尚未完成的事项</small>
      <strong>{initialBlockerFact.title}</strong>
      <p>{initialBlockerFact.orderPublicId ? `订单 ${shortPublicReference(initialBlockerFact.orderPublicId)} · ` : ''}{initialBlockerFact.reference}</p>
...
function PerformanceModule({ api, auth, view, performers, requests, phases, onChanged }: {
  ...
  return <div className="staff-module-body">
    ...
    {canManage&&<MonthlySchedulePanel api={api} auth={auth} performers={performers} onChanged={onChanged}/>}
...
function InventoryModule({ api, auth, view, onChanged }: { api: NormalizedApiClient; auth: StaffAuthView; view: InventoryView | null; onChanged(): Promise<void> }) {
  ...
  const [mode, setMode] = useState<'create' | 'edit' | 'count' | 'waste' | 'receive' | 'bind' | 'cost-correct' | null>(null)
  const [itemId, setItemId] = useState('')
  const [quantity, setQuantity] = useState('')
  const [reason, setReason] = useState('')
  const [scanCode, setScanCode] = useState('')
  const [receiptInventoryItemId, setReceiptInventoryItemId] = useState('')
  const [scannerOpen, setScannerOpen] = useState(false)
  const [batchCode, setBatchCode] = useState('')
  const [packages, setPackages] = useState('1')
  const [totalCostYuan, setTotalCostYuan] = useState('')
  const [costCorrectionYuan, setCostCorrectionYuan] = useState('')
  ...
  const [newItemBaseUnit, setNewItemBaseUnit] = useState<'ml' | 'g' | 'piece'>('ml')
  ...
  const [categoryFilter, setCategoryFilter] = useStaffViewState('inventory:category', 'all')
  ...
  const canCorrectInventoryCost = canViewInventoryCost && auth.permissions.includes('inventory.cost.correct')
  const draftReceipts = view.receipts.filter((receipt) => receipt.status === 'draft' && receipt.id !== pendingReceipt?.id)
  const receivedReceipts = view.receipts.filter((receipt) => receipt.status === 'received').slice(0, 8)
  const operationalItems = view.items
  const legacyLiquidItems = view.items.filter((item) => requiresMillilitreInventoryMigration(item.categoryCode, item.baseUnit))
  const bindableItems = view.items
  const selectedBindableItem = bindableItems.find((item) => item.id === itemId) ?? null
  const selectedReceiptItem = bindableItems.find((item) => item.id === receiptInventoryItemId) ?? null
  const selectedOperationalItem = operationalItems.find((item) => item.id === itemId) ?? null
  const selectedEditItem = view.items.find((item) => item.id === itemId) ?? null
  const selectedCostCorrectionItem = view.items.find((item) => item.id === itemId) ?? null
  const newItemUsesMillilitres = isLiquidInventoryCategory(newItemCategoryCode)

  const modeLabel: Record<Exclude<typeof mode, null>, string> = {
    create: '新建库存物料', edit: '编辑物料资料', receive: '手机扫码入库', bind: '首次绑定条码', count: '单项盘点', waste: '登记损耗', 'cost-correct': '核对后更正成本',
  }
  ...
  async function createInventoryItem(event: React.FormEvent) {
    ...
    try {
      const item = await api.postEndpoint<InventoryItemView>('/api/inventory/items', {
        sku,
        name,
        itemType: newItemUsesMillilitres
          ? 'bottle'
          : newItemBaseUnit === 'g'
            ? 'ingredient'
            : newItemCategoryCode.startsWith('food.')
              ? 'food'
              : 'consumable',
        ...
        wholeUnitCount: newItemBaseUnit === 'piece',
  ...
  async function submitInventoryAction(event: React.FormEvent) {
    ...
    const selectedItem = view?.items.find((item) => item.id === itemId)
    if (selectedItem === undefined) { setNotice('所选物料已不可用，请重新选择'); return }
    const storedQuantity = inventoryQuantityForStorage(
      quantity,
      selectedItem.categoryCode,
      selectedItem.baseUnit,
      selectedItem.packageVolumeMl,
    )
    if (storedQuantity === null) {
      setNotice(`“${selectedItem.name}”缺少有效的单瓶净含量，无法把毫升安全换算为历史库存数量；请先补全物料资料`)
      return
    }
    setBusy(true)
    setNotice('')
  ...
  async function submitCostCorrection(event: React.FormEvent) {
    event.preventDefault()
    if (busy) return
    const unitCostMinor = yuanUnitInputToMinorDecimal(costCorrectionYuan)
    const correctionReason = costCorrectionReason.trim()
    const item = view?.items.find((candidate) => candidate.id === itemId)
    if (item === undefined) { setNotice('请选择要核对成本的库存物料'); return }
    if (unitCostMinor === null) { setNotice('请填写有效的当前单位成本，最多保留八位小数'); return }
    if (correctionReason.length < 2 || correctionReason.length > 500) { setNotice('请填写2至500字的更正原因'); return }
    if (!(await confirmAction({
      title: '确认更正当前库存成本',
      description: `将“${item.name}”当前加权单位成本更正为 ¥${costCorrectionYuan.trim()}/${inventoryUnitLabel(item.baseUnit)}。这不会改写历史订单成本，但会自动重算仍在售商品和组合的后续成本。`,
      confirmLabel: '确认更正',
      tone: 'danger',
    }))) return
    setBusy(true)
    ...
    try {
      ...
      }>(`/api/inventory/items/${item.id}/cost-corrections`, {
        weightedUnitCostMinor: unitCostMinor,
        reason: correctionReason,
      }, { idempotencyKey: operationIdempotency('inventory-cost-correction') })
      setCostCorrectionYuan('')
      setCostCorrectionReason('')
      const unitLabel = inventoryUnitLabel(item.baseUnit)
      setNotice(`${item.name} 的当前单位成本已由 ${result.previousWeightedUnitCostMinor === null ? '待补' : `¥${formatInventoryUnitCostMinor(result.previousWeightedUnitCostMinor, item.baseUnit)}/${unitLabel}`} 更正为 ¥${formatInventoryUnitCostMinor(result.weightedUnitCostMinor, item.baseUnit)}/${unitLabel}${result.differenceWeightedUnitCostMinor === null ? '' : `（变动 ${formatSignedInventoryMinorDecimal(result.differenceWeightedUnitCostMinor)}/${unitLabel}）`}；后续配方和组合成本已自动重算，历史订单成本不变。`)
      await onChanged()
    } catch (error) {
      setNotice(inventoryActionMessage(error, '库存成本更正未保存'))
    } finally {
      setBusy(false)
    }
  }
  ...
  async function createReceipt(event: React.FormEvent) {
    ...
    const optionalBatchCode = batchCode.trim()
    const totalCostMinor = yuanInputToMinor(totalCostYuan)
    const directItem = view?.items.find((item) => item.id === receiptInventoryItemId) ?? null
    const usesPackageQuantity = normalizedCode !== '' || directItem?.baseUnit === 'ml'
    if (normalizedCode !== '' && (normalizedCode.length < 3 || normalizedCode.length > 128)) { setNotice('请扫描或输入有效条码'); return }
    if (normalizedCode === '' && receiptInventoryItemId === '') { setNotice('请扫描条码或选择已建档酒款'); return }
    if (!/^(?:0|[1-9]\d*)(?:\.\d{1,6})?$/.test(packages) || packages === '0') { setNotice(`请填写有效的${usesPackageQuantity ? '包装数量' : `实际入库数量（${inventoryUnitLabel(directItem?.baseUnit ?? '')}）`}`); return }
    ...
    try {
      const receipt = await api.postEndpoint<PurchaseReceiptCommandView>('/api/inventory/receipts', {
        ...
        lines: [{
          ...(normalizedCode === '' ? { inventoryItemId: receiptInventoryItemId } : { scanCode: normalizedCode }),
          batchCode: optionalBatchCode === '' ? undefined : optionalBatchCode,
          ...(usesPackageQuantity ? { packages } : { quantity: packages }),
          totalCostMinor,
          metadata: { entryMethod: normalizedCode === '' ? 'staff_mobile_selection' : 'staff_mobile_camera' },
        }],
  ...
  async function confirmReceiptAndPublish(receipt: { id: string }) {
    ...
    try {
      ...
      if (pendingReceipt?.id === receipt.id) {
        setPendingReceipt(null); setScanCode(''); setReceiptInventoryItemId(''); setBatchCode(''); setPackages('1'); setTotalCostYuan(''); setSupplierName('')
      }
      setPublishProductId('')
      setPublishPreview(null)
      await onChanged()
  ...
  return <div className="staff-module-body"><TaskSections label="库存工作" sections={[
    {id:'stock',label:'收货与库存',content:<>
    ...
    <details className="inventory-selling-guide"><summary>首次进货与上架指引</summary><section className="inventory-selling-flow" aria-label="酒水从建档到小程序可售流程"><header><div><strong>酒水四步入库发布</strong><small>同一物料可被整瓶、单杯、Shot 和鸡尾酒共同引用；发布必须先展示本次收货形成的真实成本与可售结果。</small></div><em>4 步</em></header><ol><li><b>01</b><span><strong>扫码或建立物料</strong><small>登记净含量、瓶数、采购总额、批次和供应商。</small></span>{canReceive && <button type="button" onClick={() => chooseMode('receive')}>扫码入库</button>}</li><li><b>02</b><span><strong>选择销售规格</strong><small>整瓶、单杯、Shot、鸡尾酒或自定义，均引用正式配方。</small></span>{canManageCatalog && <button type="button" onClick={() => {window.location.hash='work=products';setCatalogOpenRequest((current) => current + 1)}}>配置</button>}</li><li><b>03</b><span><strong>生成完整预览</strong><small>核对容量、单位/单份成本、扣减、可售份数、毛利和渠道。</small></span>{draftReceipts.length > 0 && <em>待预览 {draftReceipts.length}</em>}</li><li><b>04</b><span><strong>确认入库并发布</strong><small>确认前再次核对成本和可售数量，核对未通过时保留原库存。</small></span>{canManageCatalog && <button type="button" onClick={() => {window.location.hash='work=products';setCatalogOpenRequest((current) => current + 1)}}>检查商品</button>}</li></ol></section></details>
    {notice !== '' && <p className="staff-module-notice" role="status">{notice}</p>}

    {(canCount || auth.permissions.includes('inventory.count.approve')) && <InventoryStockCountPanel key={`stock-count:${auth.employee.id}`} api={api} auth={auth} refreshToken={view} onChanged={onChanged}
      onRecount={canCount ? (id) => {
        const item = operationalItems.find(candidate => candidate.id === id)
        if (item) startQuickCount(item)
        else setNotice('该物料暂不在当前库存列表，请刷新库存后核对物料状态，再提交新盘点。')
      } : undefined} />}
    {(canCount || canWaste || canReceive || canManage || canBindBarcode || canCorrectInventoryCost) && <div className="staff-module-actions" aria-label="库存操作">
      {canManage && <button type="button" className={mode === 'create' ? 'is-active' : ''} aria-pressed={mode === 'create'} onClick={() => chooseMode('create')}>新建库存物料</button>}
      ...
      {canCorrectInventoryCost && <button type="button" className={mode === 'cost-correct' ? 'is-active' : ''} aria-pressed={mode === 'cost-correct'} onClick={() => chooseMode('cost-correct')}>核对后更正成本</button>}
    </div>}
    {legacyLiquidItems.length > 0 && <p className="staff-module-footnote" role="status">有 {legacyLiquidItems.length} 项在用酒水仍保留历史“瓶”底账。员工在配方、盘点和损耗中统一填写毫升，系统按已登记的单瓶净含量兼容换算；收货包装数和单位成本仍按底账单位显示，避免改写历史流水。</p>}
    ...
    {publishPreview !== null && <section className="inventory-publish-preview" aria-label="入库发布预览"><header><div><strong>{publishPreview.productName} · {inventorySalesSpecificationLabel(publishPreview.salesSpecificationType)}</strong><small>配方第 {publishPreview.recipeVersion} 版 · 每批产出 {formatInventoryQuantity(publishPreview.yieldQuantity)} 份 · {formatReceiptReference(publishPreview.receiptId)}</small></div><em>{formatInventoryQuantity(publishPreview.sellableServings)} 份可售</em></header><div className="inventory-publish-metrics"><span><small>单份成本</small><strong>¥{formatInventoryMinor(publishPreview.costAmountMinor)}</strong></span><span><small>标准售价</small><strong>¥{formatInventoryMinor(publishPreview.standardPriceMinor)}</strong></span><span><small>单份毛利</small><strong>¥{formatInventoryMinor(publishPreview.grossProfitMinor)}</strong></span><span><small>毛利率</small><strong>{(publishPreview.marginBasisPoints / 100).toFixed(2)}%</strong></span></div><div className="inventory-publish-components">{publishPreview.components.map((component) => <article key={component.inventoryItemId}><strong>{component.itemName}</strong><span>本次增加 {formatInventoryQuantityWithUnit(component.incomingQuantity, component.baseUnit)} · 入库后可用 {formatInventoryQuantityWithUnit(component.totalAvailableAfterReceipt, component.baseUnit)}</span><span>库存单位成本 ¥{formatInventoryUnitCostMinor(component.sourceUnitCostMinor, component.baseUnit)}/{inventoryUnitLabel(component.baseUnit)}</span><span>每份扣减 {formatInventoryQuantityWithUnit(component.perServingDeduction, component.baseUnit)} · 成本 ¥{formatInventoryMinorDecimal(component.perServingCostMinor)}</span></article>)}</div><p>渠道：顾客扫码 {publishPreview.guestVisible && publishPreview.allowedChannels.includes('guest_qr') ? '可售' : '未开放'}；员工协助 {publishPreview.allowedChannels.includes('staff_assisted') ? '可售' : '未开放'}。确认时服务端会锁定并重新计算，预览不是绕过门禁的承诺。</p></section>}
    {mode === 'create' && <form className="staff-module-form inventory-create-form" onSubmit={(event) => void createInventoryItem(event)}>
      <header><strong>新建库存物料</strong><small>用于新酒款、食材、包装或配方原料。建立物料本身不增加库存；成本和数量请在后续收货单中如实登记并确认。</small></header>
      ...
      <label>库存品类<select value={newItemCategoryCode} onChange={(event) => { const categoryCode = event.target.value; setNewItemCategoryCode(categoryCode); setNewItemBaseUnit(preferredInventoryBaseUnit(categoryCode)) }}>{inventoryCategoryOptions.map(([code, label]) => <option key={code} value={code}>{label}</option>)}</select></label>
      <label>库存单位<select disabled={newItemUsesMillilitres} value={newItemBaseUnit} onChange={(event) => setNewItemBaseUnit(event.target.value as 'ml' | 'g' | 'piece')}><option value="ml">毫升（ml）</option><option value="g">克（g）</option><option value="piece">件</option></select><small>{newItemUsesMillilitres ? '该品类按毫升入库和扣减；瓶数仅作为条码包装量。' : '非液体物料按克或件管理；瓶、包等只作为包装描述。'}</small></label>
      <label>单瓶净含量<NumberInputWithUnit required={newItemUsesMillilitres} inputMode="decimal" unit="毫升/瓶" value={newItemPackageVolumeMl} onChange={(event) => setNewItemPackageVolumeMl(event.target.value)} placeholder="例如 700" /><small>按毫升库存的酒水必须填写；条码包装量会自动要求与此一致。</small></label>
      <label>安全库存（选填）<NumberInputWithUnit inputMode="decimal" unit={inventoryUnitLabel(newItemBaseUnit)} value={newItemLowStockThreshold} onChange={(event) => setNewItemLowStockThreshold(event.target.value)} placeholder={newItemBaseUnit === 'ml' ? '例如 1500' : '例如 3'} /></label>
    ...
    {mode === 'edit' && <form className="staff-module-form inventory-create-form" onSubmit={(event) => void updateInventoryItem(event)}>
      ...
      <label>库存品类<select value={newItemCategoryCode} onChange={(event) => setNewItemCategoryCode(event.target.value)}>{!inventoryCategoryOptions.some(([code]) => code === newItemCategoryCode) && <option value={newItemCategoryCode}>当前品类（{inventoryCategoryLabel(newItemCategoryCode)}）</option>}{inventoryCategoryOptions.map(([code, label]) => <option key={code} value={code}>{label}</option>)}</select></label>
      <label>当前库存单位<input readOnly value={inventoryUnitLabel(selectedEditItem?.baseUnit ?? '')} /><small>{selectedEditItem !== null && requiresMillilitreInventoryMigration(selectedEditItem.categoryCode, selectedEditItem.baseUnit) ? '历史底账继续保留瓶单位；配方、盘点和损耗会按这里登记的单瓶净含量换算成毫升供员工录入。' : '已有流水后不直接改库存单位。'}</small></label>
      <label>单瓶净含量（选填）<NumberInputWithUnit inputMode="decimal" unit="毫升/瓶" value={newItemPackageVolumeMl} onChange={(event) => setNewItemPackageVolumeMl(event.target.value)} placeholder="例如 700" /></label>
      <label>安全库存（选填）<NumberInputWithUnit inputMode="decimal" unit={inventoryUnitLabel(selectedEditItem?.baseUnit ?? '')} value={newItemLowStockThreshold} onChange={(event) => setNewItemLowStockThreshold(event.target.value)} /></label>
    ...
    {canReceive && draftReceipts.length > 0 && <section className="inventory-draft-receipts" aria-label="待确认收货单">
      ...
      {draftReceipts.map((receipt) => <article key={receipt.id}><div><strong>{formatReceiptReference(receipt.publicId)}</strong><small>{receipt.lineCount} 项 · {formatDateTime(receipt.createdAt)}</small>{canViewInventoryCost && receipt.invoiceTotalMinor !== undefined && <small>本单采购总额 ¥{formatInventoryMinorString(receipt.invoiceTotalMinor)}</small>}{!canViewInventoryCost && <small>收货金额已保存；当前账号没有“查看采购成本”权限，金额在此隐藏。</small>}{receipt.lines.map((line) => <span key={`${line.inventoryItemId}:${line.batchCode}`}>{receiptLineSummary(line, canViewInventoryCost)}</span>)}</div><div><button type="button" disabled={busy} onClick={() => void confirmReceipt(receipt)}>{busy ? '正在处理' : '仅确认入库'}</button>{canPublishBeverage && <><button type="button" disabled={busy || !publishProductId} onClick={() => void previewReceiptAndPublish(receipt)}>生成发布预览</button><button type="button" disabled={busy || publishPreview?.receiptId !== receipt.id || publishPreview.productId !== publishProductId} onClick={() => void confirmReceiptAndPublish(receipt)}>按预览确认发布</button></>}</div></article>)}
    </section>}
    {receivedReceipts.length > 0 && <section className="inventory-draft-receipts" aria-label="最近已确认收货">
    ...
    {mode === 'receive' && pendingReceipt === null && <form className="staff-module-form inventory-receipt-form" onSubmit={(event) => void createReceipt(event)}>
      ...
      <label className="inventory-code-field">商品包装条形码或二维码<div><input autoComplete="off" maxLength={128} value={scanCode} onChange={(event) => { const code = event.target.value.replace(/\s/g, ''); setScanCode(code); if (code !== '') setReceiptInventoryItemId('') }} placeholder="扫码优先；也可在下方选择物料" /><button type="button" onClick={() => setScannerOpen(true)}><ScanLine size={17} />扫码</button></div></label>
      <label>已建档物料（扫码不可用时）<select value={receiptInventoryItemId} onChange={(event) => { const next = event.target.value; setReceiptInventoryItemId(next); if (next !== '') setScanCode('') }}><option value="">不选，使用扫码</option>{bindableItems.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.packageVolumeMl === null ? inventoryUnitLabel(item.baseUnit) : `每瓶 ${formatInventoryQuantity(item.packageVolumeMl)} 毫升`}</option>)}</select></label>
      <label>采购凭证号（选填）<input maxLength={128} value={batchCode} onChange={(event) => setBatchCode(event.target.value)} placeholder="送货单号；不填则由系统自动编号" /></label>
      <label>{scanCode !== '' || selectedReceiptItem?.baseUnit === 'ml' ? '包装数量' : '实际入库数量'}<NumberInputWithUnit required inputMode="decimal" unit={scanCode !== '' || selectedReceiptItem?.baseUnit === 'ml' ? '个包装' : inventoryUnitLabel(selectedReceiptItem?.baseUnit ?? '')} value={packages} onChange={(event) => setPackages(event.target.value)} placeholder="1" /></label>
      <label>本批实际采购成本<NumberInputWithUnit required inputMode="decimal" unit="元" value={totalCostYuan} onChange={(event) => setTotalCostYuan(event.target.value)} placeholder="0.00" /><small>系统会按条码包装量和实际数量自动计算每瓶与库存单位成本，不需要填写 ¥/ml。</small></label>
    ...
    {mode === 'receive' && pendingReceipt !== null && <section className="inventory-receipt-confirm"><header><strong>待实物确认</strong><small>{formatReceiptReference(pendingReceipt.publicId)} · {pendingReceipt.lineCount} 项</small></header><p>{scanCode === '' ? `物料 ${selectedReceiptItem?.name ?? '已选物料'}` : `条码 ${scanCode}`} · 凭证 {batchCode === '' ? '系统自动编号' : batchCode} · {scanCode !== '' || selectedReceiptItem?.baseUnit === 'ml' ? `${formatInventoryQuantity(packages)} 个包装` : formatInventoryQuantityWithUnit(packages, selectedReceiptItem?.baseUnit ?? '')} · 总额 ¥{totalCostYuan}。请核对净含量、数量、采购总额和送货单；发布前系统会按这张收货单重新核算单位成本。</p><div><button type="button" disabled={busy} onClick={() => void confirmReceipt(pendingReceipt)}>{busy ? '正在处理' : '仅确认实物入库'}</button>{canPublishBeverage && <><button type="button" disabled={busy || !publishProductId} onClick={() => void previewReceiptAndPublish(pendingReceipt)}>第三步：生成完整预览</button><button type="button" disabled={busy || publishPreview?.receiptId !== pendingReceipt.id || publishPreview.productId !== publishProductId} onClick={() => void confirmReceiptAndPublish(pendingReceipt)}>第四步：按预览入库发布</button></>}</div></section>}
    {mode === 'bind' && <form className="staff-module-form inventory-bind-form" onSubmit={(event) => void bindCode(event)}>
      <header><strong>首次绑定包装条码</strong><small>只在新物料或新包装首次出现时操作。条码只能绑定一个物料，绑定后普通收货岗位可直接扫码。</small></header>
      <label>库存物料<select required value={itemId} onChange={(event) => setItemId(event.target.value)}><option value="">请选择</option>{bindableItems.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.sku} · 单位 {inventoryUnitLabel(item.baseUnit)}</option>)}</select></label>
      ...
      <label>每个条码包装计入数量<NumberInputWithUnit required inputMode="decimal" unit={`${inventoryUnitLabel(selectedBindableItem?.baseUnit ?? '')}/包装`} value={packageQuantity} onChange={(event) => setPackageQuantity(event.target.value)} /><small>例如750毫升酒瓶填750；按整件计数的物料填1。</small></label>
    ...
    {id:'products',label:'商品与上架',visible:canManageCatalog,content:<>    {canManageCatalog && <CatalogManagementPanel api={api} auth={auth} placement="inventory" openRequest={catalogOpenRequest} onOpenInventoryCost={canCorrectInventoryCost ? id=>{setItemId(id);setMode('cost-correct');setNotice('已定位所选物料；更正后可返回原商品核对成本。');openTaskSection('stock');window.setTimeout(()=>document.getElementById('inventory-cost-correction')?.scrollIntoView({behavior:'smooth',block:'center'}),0)} : undefined} />}</>},
    {id:'custody',label:'会员存酒',visible:auth.permissions.includes('bottle.manage.all'),content:<BottleCustodyPanel key={auth.employee.id} api={api} auth={auth}/>},
  ]}/></div>
}

function formatEmployeeInventoryQuantity(item: InventoryItemView): string {
  const quantity = inventoryQuantityForEmployee(
    item.availableQuantity,
    item.categoryCode,
    item.baseUnit,
    item.packageVolumeMl,
  )
  if (quantity === null) return '净含量待补，暂不能换算'
  return formatInventoryQuantityWithUnit(
    quantity,
    inventoryEmployeeUnit(item.categoryCode, item.baseUnit),
  )
}
...
function DevicesModule({ api, auth, devices, jobs, bridges, routes, onChanged }: {
  ...
  return <div className="staff-module-body">
    ...
    {jobs.some((job) => job.status === 'failed' || job.status === 'dead') && <section className="staff-song-requests"><h2>打印失败待办</h2>{jobs.filter((job) => job.status === 'failed' || job.status === 'dead').map((job) => <article key={job.id} data-staff-todo-id={`print:${job.id}`}><div><strong>{job.printerName}</strong><span>{hardwareStationLabel(job.stationCode)} · 已尝试{job.attempts}/{job.maxAttempts}次</span><span>{job.sourceReference} · {formatDateTime(job.createdAt)}</span><p>{printFailureReason(job.failureCode)}</p></div><div className="staff-inline-actions">{canRetry && job.status === 'failed' && <button type="button" disabled={busyKey !== null} onClick={() => retry(job)}>检查后重试</button>}{canReprint && <button type="button" disabled={busyKey !== null} onClick={() => reprint(job)}>核对后补打</button>}{!canRetry && !canReprint && <span>需打印处理权限</span>}</div></article>)}</section>}
    {canReprint && jobs.some((job) => job.status === 'printed') && <section className="staff-song-requests"><h2>收工补打</h2><p className="staff-module-footnote">按原票内容补打一张，票面注明“补打”；请填写原因，避免把失败任务当作补打。</p>{jobs.filter((job) => job.status === 'printed').slice(0, 20).map((job) => <article key={job.id}><div><strong>{job.sourceReference}</strong><span>{hardwareStationLabel(job.stationCode)} · {job.printerName} · {formatDateTime(job.createdAt)}</span></div><button type="button" disabled={busyKey !== null} onClick={() => reprint(job)}>补打</button></article>)}</section>}
...
function SettingsModule({ api, auth, policy, onChanged }: { api: NormalizedApiClient; auth: StaffAuthView; policy: CommercePolicyView | null; onChanged(): Promise<void> }) {
  ...
  return <div className="staff-module-body">
    ...
    {(auth.permissions.includes('customer.public-profile.manage')
      ...
      || auth.permissions.includes('privacy.policy.publish')) && <details className="staff-module-disclosure"><summary>管理员发布设置</summary><CustomerPublicationPanel api={api} permissions={auth.permissions} /></details>}
  </div>
}
...
function inventoryView(value: unknown): InventoryView | null {
  ...
  return {
    items: value.items.flatMap((item) => isRecord(item)
      ...
      && (item.costStatus === 'complete' || item.costStatus === 'pending' || item.costStatus === 'needs_review')
  ...
  return value.flatMap((entry): PurchaseReceiptView[] => {
    ...
    const lines = entry.lines.flatMap((line) => isRecord(line)
      ...
      ? [{ inventoryItemId: line.inventoryItemId, itemName: line.itemName, batchCode: line.batchCode, quantity: line.quantity, baseUnit: line.baseUnit,
        ...(typeof line.packageCount === 'string' ? { packageCount: line.packageCount } : {}),
        ...(typeof line.packageVolumeMl === 'string' || line.packageVolumeMl === null ? { packageVolumeMl: line.packageVolumeMl } : {}),
        ...(typeof line.unitCostMinor === 'string' ? { unitCostMinor: line.unitCostMinor } : {}),
        ...(typeof line.perPackageCostMinor === 'string' ? { perPackageCostMinor: line.perPackageCostMinor } : {}),
        ...(typeof line.totalCostMinor === 'string' ? { totalCostMinor: line.totalCostMinor } : {}),
      }] : [])
    return [{ id: entry.id, publicId: entry.publicId, status: entry.status, lineCount: entry.lineCount, lines, createdAt: entry.createdAt, receivedAt: entry.receivedAt,
      ...(typeof entry.invoiceTotalMinor === 'string' || entry.invoiceTotalMinor === null ? { invoiceTotalMinor: entry.invoiceTotalMinor } : {}),
    }]
  })
}
...
function preferredInventoryBaseUnit(code: string): 'ml' | 'g' | 'piece' {
  if (isLiquidInventoryCategory(code)) return 'ml'
  if (code.startsWith('ingredient.')) return 'g'
  return 'piece'
}
...
function formatInventoryMinorString(value: string | null): string {
  if (value === null || !/^\d+$/.test(value)) return '待确认'
...
function formatSignedInventoryMinorDecimal(value: string): string {
  const parsed = Number(value)
  if (!Number.isFinite(parsed)) return '待确认'
  return `${parsed > 0 ? '+' : ''}¥${formatInventoryMinorDecimal(value)}`
}

function receiptLineSummary(line: PurchaseReceiptView['lines'][number], canViewCost: boolean): string {
  const packaging = line.packageCount === undefined
    ? ''
    : ` · ${formatInventoryQuantity(line.packageCount)} 个包装${line.packageVolumeMl === null || line.packageVolumeMl === undefined ? '' : `（每个 ${formatInventoryQuantity(line.packageVolumeMl)} 毫升）`}`
  const costs = !canViewCost
    ? ''
    : `${line.totalCostMinor === undefined ? '' : ` · 本批 ¥${formatInventoryMinorString(line.totalCostMinor)}`}${line.perPackageCostMinor === undefined ? '' : ` · 每个包装 ¥${formatInventoryUnitCostMinor(line.perPackageCostMinor, 'piece')}`}${line.unitCostMinor === undefined ? '' : ` · 单位成本约 ¥${formatInventoryUnitCostMinor(line.unitCostMinor, line.baseUnit)}/${inventoryUnitLabel(line.baseUnit)}`}`
  return `${line.itemName} · 凭证 ${line.batchCode}${packaging} · 实际入库 ${formatInventoryQuantityWithUnit(line.quantity, line.baseUnit)}${costs}`
}
...
function yuanUnitInputToMinorDecimal(value: string): string | null {
  const normalized = value.trim()
  if (!/^(?:0|[1-9]\d*)(?:\.\d{1,8})?$/.test(normalized)) return null
  const [yuan = '0', fraction = ''] = normalized.split('.')
  const wholeMinor = BigInt(yuan) * 100n + BigInt(`${fraction}00`.slice(0, 2))
  const fractionalMinor = fraction.slice(2).padEnd(6, '0')
  return fractionalMinor === '000000'
    ? wholeMinor.toString()
...
function hardwareStationLabel(value: string): string { return ({ bar: '吧台', kitchen: '后厨', cashier: '收银台' } as Record<string, string>)[value] ?? '位置待核对' }
