...
Page({
  ...
  async load() {
    ...
    try {
      ...
      this.setData({
        ...
        displayName: profile.displayName || runtime.getStorageSync('mbox.member.displayName') || '',
  ...
  onChooseAvatar() {
    runtime.chooseImage({
      count: 1,
      success: (result) => {
        const paths = result && (result.apFilePaths || result.tempFilePaths) || []
        const avatarUrl = String(paths[0] || '')
        if (!avatarUrl) return
        runtime.setStorageSync(AVATAR_KEY, avatarUrl)
        this.setData({ avatarUrl })
      },
      fail: () => runtime.showToast({ title: '未选择头像', icon: 'none' }),
    })
  },
  ...
  async save() {
    ...
    try {
      ...
      runtime.setStorageSync('mbox.member.displayName', displayName || '')
      runtime.showToast({ title: '已保存', icon: 'success' })
      setTimeout(() => runtime.navigateBack(), 550)
    } catch (error) {
