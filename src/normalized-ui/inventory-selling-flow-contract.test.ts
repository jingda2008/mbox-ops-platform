...
describe('inventory-to-sale workflow contract', () => {
  it('keeps beverage stock, product setup and sale checks in one guided workspace', () => {
    ...
    expect(catalog).toContain('套餐成本与最低毛利')
    expect(catalog).toContain('calculateBundleCostRange')
    expect(catalog).toContain('实际订单会按客人逐份选择重新计算，不使用套餐平均成本')
    expect(catalog).toContain('系统不会用零成本推算毛利')
    ...
    const waste = read('src/normalized-ui/InventoryWastePanel.tsx')
    expect(waste).toContain('液体统一填写毫升')
    expect(waste).toContain('inventoryQuantityForStorage(')
    expect(catalog).not.toContain('第 5 步')
    expect(catalog).toContain('第 2 步：选择销售规格并配置配方')
