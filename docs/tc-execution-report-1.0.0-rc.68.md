...
另有 **27条** `TME/CAP/RPF` 增量专项TC；其失败、未通过、未执行、待执行或阻塞项同样阻止商业生产发布，状态以专项文件和不可变运行证据为准。

| 状态 | 数量 | 判定口径 |
|---|---:|---|
| 通过 | 146 | 既有基线判定；其中只有标记“直接测试映射”的项目具备逐条代码引用 |
| 未执行 | 50 | 必须由真实岗位、真实设备或门店现场完成，不能用单元测试冒充 |
| 阻塞 | 17 | 缺真实支付参数、外部流水或第三方生产通道 |
| 失败 | 0 | 已执行但结果不符合预期；本轮为0 |

**商业生产发布结论：不通过发布门禁。** 当前仍有 **67条 P0/P1** 未达到“通过”：13条P0阻塞、10条P0未执行、4条P1阻塞、40条P1未执行。
