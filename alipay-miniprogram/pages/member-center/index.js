...
Page({
  ...
  async claimDailySnack() {
    ...
    try {
      ...
      runtime.showToast({ title: '已生成核销码', icon: 'success' })
    } catch (error) {
  ...
  openPoints() { runtime.navigateTo({ url: '/pages/points/index' }) },
  openCoupons() { runtime.navigateTo({ url: '/pages/profile-coupons/index' }) },
  backToProfile() { runtime.switchTab({ url: '/pages/profile/index' }) },
})
