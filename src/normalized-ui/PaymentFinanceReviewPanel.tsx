...
export function PaymentFinanceReviewPanel({api,canManage}:{api:NormalizedApiClient;canManage:boolean}){
 ...
 const phase=(row:Row)=>row.financialSignals?.includes('confirmed_payment_not_applied')?'已确认付款，待恢复账务':['created','pending'].includes(row.status)?row.phase==='stopped'?'自动查询已停止，待财务核对':row.nextQueryAt?`后台继续核对；下次 ${time(row.nextQueryAt)}`:'支付结果未知，待财务核对':'支付已取得最终状态，待核对结案'
