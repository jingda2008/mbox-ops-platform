...
try {
  ...
  if ($manifest.version -ne '1.0.5' -or $manifest.packageRevision -ne 'r1') { throw '升级包版本不正确。' }
  ...
  # Exact normalized bytes of the 1.0.4 release package. Unknown adaptations are never overwritten.
  $originalHashes = @{
    'bridge.mjs'='c5d52d59d4bd654649d1770ccd332937e31a1d2e471de63a0823acf1a96b4212'
    'list-printers.ps1'='b317985f5812785d32ef8fb3c149969e26d3b811cf5e5a3bf2244403636ffb26'
    'print-ticket.ps1'='5b4a8a1335978ccf56d44a42d93d4b73be1b8b36fe53c19bba7cf8da1a6bc7e7'
  }
  $validatedHashes = @{}
  $operations = @{
    Validate = {
      ...
      if ([version]$Matches[1] -gt [version]'1.0.5') { throw '本包不会降级更新的版本。' }
      $same = $true
      ...
      foreach ($file in $files) {
        ...
        if (-not $same -and $hash -ne $originalHashes[$file]) { throw '原程序不符合1.0.4标准包或本包，停止覆盖；请勿强制安装。' }
  ...
  $record = [pscustomobject]@{ Time=(Get-Date).ToString('o'); Package='1.0.5-r1'; Result=$result; Backup=$backup; BackendHeartbeat='not-verified'; PhysicalPrinting='not-verified' }
  ...
  if ($result.State -eq 'failed') {
    ...
    $message = '打印程序已核验为1.0.5，服务正在运行。' + [Environment]::NewLine + '原配对和打印机配置保留。还需后台确认新心跳为1.0.5，并核对两台实体票。' + [Environment]::NewLine + "备份：$backup"
    $exitCode = 0
  }
