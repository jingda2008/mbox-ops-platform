...
Page({
  ...
  async loadActiveData(request) {
    const results = await Promise.all([
      ...
      getAlipayNotificationPrompt('order_checkout').catch(() => ({ available: false, authorizations: [] })),
      getAlipayNotificationPrompt('order_selection').catch(() => ({ available: false, authorizations: [] })),
    ...
    this.setData({
      ...
      membershipInviteVisible: false,
      alipayNotificationPromptOptions: extractPromptPresentation(results[6]),
      alipayOrderSelectionPromptOptions: extractPromptPresentation(results[7]),
  ...
  async clearCart() {
    ...
    try {
      ...
      if (error && error.code === 'WECHAT_IDENTITY_REQUIRED') {
        runtime.removeStorageSync(CHECKOUT_ATTEMPT_KEY)
        this.setData({
          error: '当前后端支付身份协议不适用于支付宝，本次没有创建订单或占用商品；请联系服务员。',
          checkoutLocked: false,
        })
        return
      }
  ...
  async openCheckout() {
    ...
    if (!alipayOnlinePaymentEnabled()) {
      this.setData({ error: '支付宝在线支付后端适配尚未接通，本次没有创建订单；请联系服务员点单或结账。' })
      return
    }
    this.setData({ checkoutConfirmVisible: true, cartExpanded: false, error: '' })
  },
