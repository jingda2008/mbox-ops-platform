...
test('one business-day order and guest requests flow through bartender, kitchen, service and manager roles', async ({ browser }) => {
  ...
  await expect(cashier.page.getByLabel('本营业日售后摘要')).toContainText('1订单')
  ...
  await expect(pendingOrder).toContainText('模拟支付')
  await expect(pendingOrder).toContainText('剩余可退 ¥0.00')
