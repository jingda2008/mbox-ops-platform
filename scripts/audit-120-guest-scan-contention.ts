...
if(!url||new URL(url).hostname!=='127.0.0.1'||new URL(url).pathname!=='/mbox120_guest')throw new Error('Requires dedicated local mbox120_guest audit database')
...
try{
  ...
  await writeFile('artifacts/audit-120-guest-20260921/scan-contention.json',JSON.stringify({source:'77744bc4e9ed308546b630590bdfa5dfe5ce5b7d',mode:'real-service-postgres-synthetic-tables',pool:12,results},null,2))
