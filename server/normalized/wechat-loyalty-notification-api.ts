...
export const wechatLoyaltyNotificationApiPlugin: FastifyPluginAsync<WechatLoyaltyNotificationApiOptions> = async (
  app,
  options,
) => {
  app.get('/public/mini/wechat-notification-authorizations', async (request, reply) => {
    ...
    const authorizations = await options.transactions.run(context.scope, (transaction) => (
      new WechatLoyaltyNotificationRepository(transaction).authorizationOptions(
        context.customerId,
  ...
  app.post('/public/mini/wechat-notification-authorizations', async (request, reply) => {
    ...
    const input = {
      ...
      policyId: uuid(body.policyId, '通知政策'),
    ...
    try {
      const execution = await options.commands.execute({
        scope: context.scope,
        operationScope: 'customer.wechat-notification-authorization.record',
        idempotencyKey,
        requestFingerprint: fingerprint({ ...input, customerId: context.customerId }),
        resultCodec: authorizationResultCodec,
      }, async (transaction) => {
        const recorded = await new WechatLoyaltyNotificationRepository(transaction).recordAuthorization({
          customerId: context.customerId,
          ...input,
        })
        const result: AuthorizationResult = {
          id: recorded.id,
          notificationType: recorded.notificationType,
        ...
        return {
          ...
          auditEvents: [{
            ...
            action: 'customer.wechat-notification-authorization.recorded',
            objectType: 'wechat_notification_authorization',
            objectId: recorded.id,
            businessDate: context.businessDate,
            metadata: {
              notificationType: recorded.notificationType,
              purpose: recorded.purpose,
              authorizationContext: recorded.authorizationContext,
      ...
      if (error instanceof WechatNotificationAuthorizationError) {
        const status = error.code === 'WECHAT_NOTIFICATION_MEMBERSHIP_REQUIRED' ? 409
          : error.code === 'WECHAT_NOTIFICATION_IDENTITY_REQUIRED' ? 403
            : 409
...
const authorizationResultCodec: JsonCodec<AuthorizationResult> = {
  encode: (value) => ({ ...value }),
  decode: (value) => {
    const row = object(value)
    return {
      id: uuid(row.id, '授权'),
      notificationType: enumValue(row.notificationType, WECHAT_LOYALTY_NOTIFICATION_TYPES, '通知类型'),
...
function enumValue<const Value extends string>(value: unknown, values: readonly Value[], label: string): Value {
  if (typeof value !== 'string' || !values.includes(value as Value)) throw new TypeError(`${label}格式不正确`)
