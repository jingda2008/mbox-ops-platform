...
describe('pilot staff reconciliation', () => {
  it('adds only missing staff and shifts, repairs canonical duties and remains idempotent', () => {
    ...
    state.config.roles.find((role) => role.id === 'owner')!.name = '老板'
    state.config.roles = state.config.roles.filter((role) => role.id !== 'market_operations')
    const previousRevision = state.revision

    const first = reconcilePilotStaff(state, createSeedState(), '2026-07-15T01:00:00.000Z')
    expect(first).toMatchObject({
      changed: true,
      addedRoleIds: ['market_operations'],
      updatedRoleIds: ['owner'],
      addedEmployeeIds: expect.arrayContaining([
      'emp-owner', 'emp-admin', 'emp-han', 'emp-tao', 'emp-cashier', 'emp-host',
      ]),
    })
    expect(state.config.roles.find((role) => role.id === 'owner')?.name).toBe('运营负责人')
