...
for(const fail of [false,true]) test(`ingress convergence ${fail?'keeps legacy running and restores routes when probe fails':'verifies both routes before retiring legacy runtime'}`,async()=>{
 ...
 try{
  ...
  const original='https://139.224.254.60 {\n reverse_proxy mbox-app:8787\n}\n'
  await writeFile(config,original.replaceAll('\n','\r\n'));await writeFile(snippet,original);await writeFile(watchdog,'MBOX_REQUIRED_CONTAINERS="mbox-app mbox-caddy"\n')
...
`,{mode:0o700})
  ...
  if(fail){assert.notEqual(result.status,0);await assert.rejects(access(join(dir,'stopped')));assert.equal(await readFile(config,'utf8'),original.replaceAll('\n','\r\n'));assert.match(await readFile(watchdog,'utf8'),/mbox-app mbox-caddy/)}
  else{assert.equal(result.status,0,result.stderr);await access(join(dir,'stopped'));assert.equal(JSON.parse(await readFile(join(root,'observability/legacy-runtime-retired.json'),'utf8')).legacyStopped,true);assert.match(await readFile(snippet,'utf8'),/tls_server_name mbox.shmbox.com/);assert.doesNotMatch(await readFile(config,'utf8'),/139\.224\.254\.60/);assert.doesNotMatch(await readFile(config,'utf8'),/reverse_proxy mbox-app/);assert.equal(await readFile(watchdog,'utf8'),'\nMBOX_REQUIRED_CONTAINERS="mbox-caddy"\n')}
