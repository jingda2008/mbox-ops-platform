...
test('mobile manager assists an open table order and gifts a product without mixing member benefits', async ({ page }) => {
  ...
  await paidSheet.getByLabel('搜索点单商品').fill(data.orderableProductName)
  await paidSheet.getByRole('button', { name: `添加${data.orderableProductName}` }).click()
  await paidSheet.getByRole('button', { name: '核对无误，确认下单' }).click()
