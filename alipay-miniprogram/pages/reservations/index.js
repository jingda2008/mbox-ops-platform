...
} = require('../../utils/alipay-subscription-presentation-cache')
...
Page({
  ...
  async enablePerformanceNotification(event) {
    if (getRuntimeConfig().alipayNotificationEnabled !== true) {
      this.setData({ notificationNotice: '支付宝消息提醒后端适配尚未接通，当前不会申请授权。' })
      return
    }
    ...
    if (!option || this.data.notificationBusyId || typeof runtime.requestSubscribeMessage !== 'function') return
    ...
    try {
      ...
      const result = await requestAlipaySubscriptionFromTap(options, RESERVATION_SUCCESS_SUBSCRIBE_TYPES)
  ...
  async cancelReservation(event) {
    ...
    const confirmed = await new Promise((resolve) => {
      runtime.showModal({
        ...
        fail: (error) => {
          runtime.showToast({ title: (error && error.errMsg) || '确认弹窗未能打开', icon: 'none' })
          resolve(false)
        },
      })
    })
  ...
  nextStep() {
    ...
    this.setData({ step, error: '' }, () => {
      if (step === 3) this.preloadAlipaySubscriptionPresentationOptions().catch(() => {})
    })
  },
  ...
  async preloadAlipaySubscriptionPresentationOptions() {
    if (getRuntimeConfig().alipayNotificationEnabled !== true) {
      ...
      this.setData({ alipaySubscriptionPresentationOptions: [] })
      return []
    }
    try {
      ...
      const [performance, reservationPrompt] = await Promise.all([
        ...
        getAlipayNotificationPrompt('reservation_submit').catch(() => empty),
      ])
  ...
  submitReservation() {
    ...
    requestAlipaySubscriptionFromTap(options, RESERVATION_SUCCESS_SUBSCRIBE_TYPES).finally(() => {
