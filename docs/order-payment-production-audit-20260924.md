...
当前修复状态：2026-09-24 11:10 CST已部署rc.231，权限时序和历史状态说明已修复；实际取餐设备绑定及实物闭环仍待验收。以下保留审计与修复过程。
...
## 2026-09-24 11:13 CST 生产修复结果

- PR294合并；生产SHA d55187768ad2680da1db39b436e5dc347e5ec28d，rc.231/schema245，无迁移。PR CI35947776245、标签CI35948709872和发布35948709862均成功。
- 标准发布状态completed；镜像摘要sha256:cac1e13423b2cd63f01a4470669bee84e8ba564fe049894c6e52c1843f1313a1。备份和旧镜像回退保留；证据/镜像/备份/完成记录四项OSS回读verified=true，公网四入口和浏览器验证通过。
- 已部署代码的实际受限账号只读调用确认微秒鉴权、取餐账号仅kds.deliver、原4笔8行历史说明可读；4笔原订单及收退款事实SHA256指纹保持不变、欠款0。
- SYS-377关闭；SYS-376代码部分上线，启用取餐设备仍0，实际平板绑定及真实取走/撤回继续待验收。未进行生产模拟下单、再次收退款或补写送达。
- 初次默认Bash4.2环境预检失败即停，复用既有Bash5.2后成功，未放宽发布门禁。受限证据：rc231-deploy.log（首次失败）、rc231-deploy-retry.log（成功）、rc231-completion.json、rc231-ready.json、post-release-runtime.json、post-release-financial-comparison.json。
