...
initial='5cd9f2993e845a2049484a5e1eab6d5b67718d26';final=initial;old='5b9d929499b1d8cb0eb3a0c0668604e9a398f1fe'
...
def fetch(version,commit):
 ...
 return dest,m
old_dir,old_manifest=fetch('1.0.0-rc.216',old)
run(['docker','tag',old_manifest['imageTag'],'audit-maintenance-source-live:5b9d929'])
assets=root/'assets';assets.mkdir();shutil.copytree(fixture,assets/'fixture');shutil.copytree(old_dir,assets/'old-release')
unpack_source(initial,assets/'source-5cd9f29')
...
run(['docker','save','--output',assets/'initial-image.tar',initial_tag])
...
images={initial:{'tag':initial_tag,'digest':exported_digest,'config':config,'archive':'initial-image.tar'}};(assets/'images.json').write_text(json.dumps(images));(artifacts/'input-identities.json').write_text(json.dumps({'source':old_manifest['releaseSha'],'images':images,'nativeArchitecture':os.uname().machine,'productionCredentialsUsed':False},indent=2)+'\n')
...
inner_images=root/'inner-images.tar';run(['docker','save','-o',inner_images,'audit-maintenance-source-live:5b9d929',initial_tag,'caddy:2.10.2-alpine'])
# Candidate-only normal entry; final official-image forward recovery remains a separate gate.
...
print('Candidate-only normal maintenance entry verified; official final-image forward scenario remains pending')
