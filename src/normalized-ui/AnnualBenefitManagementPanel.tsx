import {benefitKindLabel} from '../shared/staff-business-labels'
...
interface DailySnackClaim { currentFulfillmentStatus?: 'pending'|'ready'|'delivered'|'cancelled'|null; id: string; claimCode: string; quantity: number; status: 'reserved' | 'redeemed' | 'fulfilled' | 'cancelled' | 'expired' | 'cancelled_after_redemption' | 'compensated'; expiresAt: string | null; redeemedByEmployeeName: string | null; redeemedAt: string | null; fulfilledAt: string | null; title: string; tableCode?: string }
...
export function AnnualBenefitManagementPanel({ api, auth, viewMode='all' }: { api: NormalizedApiClient; auth: StaffAuthView; viewMode?:'all'|'fulfillment'|'exceptions' }) {
  const { confirmAction, promptAction } = useConfirmationDialog()
  const canView = viewMode==='all' && auth.permissions.includes('loyalty.annual-benefit.view')
  const canManage = viewMode==='all' && auth.permissions.includes('loyalty.annual-benefit.manage')
  const canApprove = viewMode==='all' && auth.permissions.includes('loyalty.annual-benefit.approve')
  const canPublish = viewMode==='all' && auth.permissions.includes('loyalty.annual-benefit.publish')
  const canConfirmOccurrence = viewMode==='all' && auth.permissions.includes('loyalty.annual-benefit.occurrence.confirm')
  ...
  async function publish(policy: Policy) {
    const effectiveFrom = (await promptAction({title:'填写未来生效时间',description:'请选择北京时间。',inputType:'datetime-local',label:'生效时间',confirmLabel:'继续',multiline:false}))?.trim() ?? ''
  ...
  return <section className="staff-module-summary annual-benefit-panel"><div>
    ...
    {canManage && <form className="staff-module-form" onSubmit={(event) => void saveDraft(event)}>
      ...
      <fieldset className="activity-wide"><legend>添加年度礼遇规则</legend>
        ...
        <label>权益定义<select value={draftRule.benefitDefinitionId} onChange={(event) => updateRule({ benefitDefinitionId: event.target.value })}><option value="">请选择</option>{configuration?.definitions.filter((item) => item.status === 'active').map((item) => <option key={item.id} value={item.id}>{item.name}（{benefitKindLabel(item.benefitKind)}）</option>)}</select></label>
