...
test('mobile guest scans a fixed table QR, searches, orders and sees payment result', async ({ page }) => {
  ...
  const detailEntry = page.getByRole('button', { name: `查看${data.orderableProductName}详情` }).last()
  await expect(detailEntry).toBeVisible()
  await detailEntry.click()
  const productDetail = page.getByRole('dialog', { name: `${data.orderableProductName}商品详情` })
  await productDetail.getByRole('button', { name: '加入购物车' }).click()
  await expect(productDetail.getByLabel(`${data.orderableProductName}已选1件`)).toBeVisible()
  await productDetail.getByTitle('关闭商品详情').click()
  ...
  await page.getByRole('button', { name: `查看${data.orderableProductName}详情` }).last().click()
  const continuationProductDetail = page.getByRole('dialog', { name: `${data.orderableProductName}商品详情` })
  await continuationProductDetail.getByRole('button', { name: '加入购物车' }).click()
  await page.getByRole('dialog', { name: '确认继续加单' }).getByRole('button', { name: '继续选商品' }).click()
  await continuationProductDetail.getByTitle('关闭商品详情').click()
  await page.getByRole('button', { name: /核对订单/ }).click()
...
test('mobile manager assists an open table order and gifts a product without mixing member benefits', async ({ page }) => {
  ...
  await paidSheet.getByRole('button', { name: `查看${data.orderableProductName}详情` }).last().click()
  const paidProductDetail = paidSheet.getByRole('dialog', { name: `${data.orderableProductName}商品详情` })
  await paidProductDetail.getByRole('button', { name: '加入购物车' }).click()
  await paidProductDetail.getByTitle('关闭商品详情').click()
