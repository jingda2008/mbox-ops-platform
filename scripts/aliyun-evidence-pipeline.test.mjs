...
test('formal deployment requires OSS evidence before activation and uploads database backup first', async () => {
  ...
  const relayBackupUpload = deploy.indexOf('preverified-backup-upload.json')
  const remoteActivation = deploy.lastIndexOf("'${remote_release_dir}/activate-release.sh'")
  assert.ok(relayBackupUpload > 0 && relayBackupUpload < remoteActivation)
  assert.match(deploy, /MBOX_EVIDENCE_SSH_HOST/)
  assert.match(deploy, /evidence-relay/)
  assert.match(deploy, /backup-postgres\.sh' prepare-relay/)
  assert.match(deploy, /preverified-backup-upload\.json/)
  assert.match(activate, /relay_backup_verified/)
  assert.match(activate, /databaseIdentity[\s\S]*candidate_database_identity/)
  assert.match(activate, /\.authMode == "EcsRamRole"[\s\S]*\.completionMarker/)
