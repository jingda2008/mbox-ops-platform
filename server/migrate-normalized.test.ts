...
describe('normalized migration baseline', () => {
  ...
  it('adds exact-count bundle choices and per-unit shared-cart selections', async () => {
    const migration = (await loadNormalizedMigrations()).find((entry) => entry.version === '159')
    expect(migration?.filename).toBe('159_bundle_choice_groups.sql')
    expect(migration?.sql).toMatch(/CREATE TABLE mbox\.product_bundle_choice_groups/)
    expect(migration?.sql).toMatch(/CREATE TABLE mbox\.product_bundle_choice_options/)
    expect(migration?.sql).toMatch(/ADD COLUMN bundle_selections jsonb NOT NULL/)
    expect(migration?.sql).toMatch(/command IN \('adjust','replace_selection','remove','clear','submit','expire'\)/)
  ...
  it('permits a reversible recommendation pilot from a current legacy policy but keeps full enablement managed', async () => {
    const migration = (await loadNormalizedMigrations()).find((entry) => entry.version === '150')
    expect(migration?.sql).toMatch(/rollout_state='pilot'/)
    expect(migration?.sql).toMatch(/current published policy/)
    expect(migration?.sql).toMatch(/rollout_state='enabled'/)
    expect(migration?.sql).toMatch(/current managed three-person policy/)
    expect(migration?.sql).toMatch(/schema_version='150'/)
  })
  ...
  it('lets cashiers assist ordering and safely release a table without catalog or gift authority', async () => {
    const migration = (await loadNormalizedMigrations()).find((entry) => entry.version === '157')
    expect(migration?.filename).toBe('157_cashier_assisted_order_and_safe_turnover.sql')
    expect(migration?.sql).toMatch(/permission\.code IN \('order\.create','table\.view_all','table\.close','table\.turnover_unsettled'\)/)
    expect(migration?.sql).toMatch(/role\.code='CASHIER'/)
    expect(migration?.sql).not.toMatch(/catalog\.product\.manage|order\.gift/)
    expect(migration?.sql).toMatch(/schema_version='157'/)
  })
