...
describe('inventory-to-sale workflow contract', () => {
  it('keeps beverage stock, product setup and sale checks in one guided workspace', () => {
    ...
    expect(staff).toContain('酒水从建档到小程序可售')
    expect(staff).toContain('建立物料')
    expect(staff).toContain('绑定并扫码收货')
    expect(staff).toContain('确认实际库存')
    expect(staff).toContain('建立商品与售价')
    expect(staff).toContain('配置配方并小程序上架')
