...
type DisplayMode='pinned'|'rotation'
interface CardView{
  code:string;type:CardType;title:string;summary:string;imageUrl:string|null;ctaLabel:string
  targetPath:string;priority:number;displayMode:DisplayMode;visibility:'public'|'member'|'segment'
...
interface Draft{
  ...
  targetPath:string;priority:string;displayMode:DisplayMode;visibility:'public'|'member';validFrom:string;validUntil:string;reason:string
}
...
export function HomeContentManagementPanel({api,auth}:{api:NormalizedApiClient;auth:StaffAuthView}){
  ...
  async function action(card:CardView,operation:'publish'|'pause'){
    ...
    try{
      ...
      setNotice(operation==='publish'?'内容已发布；无需重新发版。':'内容已从首页撤下，可修改后重新发布。');await load()
    }catch(error){setNotice(message(error,operation==='publish'?'内容没有发布':'内容没有撤下'))}
    finally{setBusy('')}
  }
    ...
    {expanded&&<>
      ...
      {draft&&canManage&&<details className="activity-draft-editor" open><summary>{selected?'编辑首页内容草稿':'填写新内容'}</summary><form onSubmit={event=>void save(event)}>
        <fieldset><legend>展示内容</legend>
          ...
          <label className="wide">图片（可选）<input readOnly value={draft.imageUrl} placeholder="请通过下方图片库上传或选择（单张不超过 200KB）" /></label>
        ...
        <fieldset><legend>操作与排期</legend>
          ...
          <label>首页展示方式<select value={draft.displayMode} onChange={event=>update('displayMode',event.target.value as DisplayMode)}><option value="pinned">常驻首页</option><option value="rotation">轮换展示</option></select><small>常驻内容始终保留；轮换内容每 6 小时选出一条。暂停展示可立即撤下。</small></label>
...
function emptyDraft():Draft{const start=new Date();start.setSeconds(0,0);const end=new Date(start.getTime()+30*86400000);return{code:'',type:'article',title:'',summary:'',imageUrl:'',ctaLabel:'查看内容',targetPath:'/pages/home/index',priority:'100',displayMode:'rotation',visibility:'public',validFrom:local(start),validUntil:local(end),reason:''}}
...
function status(value:CardStatus){return({draft:'草稿',published:'展示中',paused:'已撤下',retired:'已退役'} as const)[value]}
function displayMode(value:DisplayMode){return value==='pinned'?'常驻首页':'轮换展示'}
