...
if (qualitySupplementCases.length !== 63) {
  throw new Error(`Expected 63 time/capacity/performance/data-integrity TCs, found ${qualitySupplementCases.length}`)
}
...
const expectedQualitySupplementIds = [
  ...
  ...Array.from({ length: 26 }, (_, index) => `CAP-${String(index + 1).padStart(3, '0')}`),
  ...Array.from({ length: 17 }, (_, index) => `RPF-${String(index + 1).padStart(3, '0')}`),
  ...Array.from({ length: 7 }, (_, index) => `DAT-${String(index + 1).padStart(3, '0')}`),
]
