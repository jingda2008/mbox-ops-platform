...
test('60单180份：低高度双板、真实开批、跨页草稿、出锅释放及取送状态',async({page},testInfo)=>{
  ...
  await board(page).getByRole('button',{name:'隔离炸篮 A 已清空'}).click()
  await expect(board(page).getByRole('status',{name:'后厨操作反馈',exact:true})).toContainText('已确认设备清空')
  const right=board(page).getByRole('region',{name:'正在制作',exact:true})
  // First batch selection defaults to all portions. Preserve that contract,
  // then explicitly choose one portion on each page to exercise draft recovery.
  await expect.poll(async()=>Number(await right.getByRole('spinbutton').first().inputValue())).toBeGreaterThan(0)
  for(const input of await right.getByRole('spinbutton').all())await input.fill('0')
  ...
  await right.getByRole('button',{name:'本页 1 份已放好'}).click()
  await expect(board(page).getByRole('status',{name:'后厨操作反馈',exact:true})).toContainText('已确认 1 份放好')
  await right.getByRole('button',{name:'上一页',exact:true}).click()
  ...
  await board(page).getByLabel('找桌 / 品名 / 订单').fill('k03')
  await expect(right.getByRole('spinbutton').first()).toHaveAccessibleName(/K03/)
  await board(page).getByLabel('找桌 / 品名 / 订单').fill('')
  await board(page).getByRole('region',{name:'待制作',exact:true}).getByRole('button').filter({hasText:'隔离合批薯条'}).first().click()
  ...
  const whole=right.getByRole('button',{name:'整批 16 份已放好',exact:true});await expect(whole).toBeVisible()
...
test('服务员读取真实备齐数量并送达；厨师操作不冒充送达',async({page,browser})=>{
  ...
  await page.goto('/')
  const notice=page.getByRole('status',{name:'出品待取提醒',exact:true})
  await expect(notice).toContainText('份出品待取')
  ...
  try{
    ...
    const response=await cook.request.post('/api/commerce/kitchen-board/commands',{headers:{'idempotency-key':crypto.randomUUID()},data:{employeeId:queue.employeeId,command:{action:'ready',batchId:batch.id,expectedOwnershipVersion:batch.ownershipVersion,items:[{taskId:next.taskId,tableId:next.tableId,tableSessionId:next.tableSessionId,locationVersion:next.locationVersion,unitIds:[next.unitId]}]}}})
    expect(response.ok(),await response.text()).toBe(true)
    await expect(notice).toContainText('3 份出品待取')
  }finally{await cookContext.close()}

  await notice.getByRole('button',{name:'查看出品'}).click()
  await page.getByRole('button',{name:/^待取（/}).click()
