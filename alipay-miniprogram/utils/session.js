...
function applyLaunchSession(options, config) {
  ...
  const previous = runtime.getStorageSync('mbox.table.session') || {}
  const connection = runtime.getStorageSync('mbox.table.connection.state') || {}
  ...
  runtime.setStorageSync('mbox.table.session', session)
  return session
}
...
function getTableSession() {
  const app = getApp()
  return app.globalData.tableSession || runtime.getStorageSync('mbox.table.session') || {}
}
...
function rememberTableConnection(value) {
  ...
  runtime.setStorageSync('mbox.table.connection.state', state)
...
function getTableConnection() {
  return runtime.getStorageSync('mbox.table.connection.state') || { status: 'unknown' }
}

function clearTableConnection() {
  runtime.removeStorageSync('mbox.connected.table.token')
  runtime.removeStorageSync('mbox.table.connection.state')
  runtime.removeStorageSync('mbox.http.cookie.guest.v2')
