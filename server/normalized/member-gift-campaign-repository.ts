...
export class MemberGiftCampaignRepository{
  ...
  async deliver(jobId:string){
    ...
    const benefit=await new BenefitRepository(this.tx).issue({customerId,benefitCode:campaign.code,quantity:job.quantity,couponCalendarVersionId:campaign.rule.couponCalendarVersionId,
      ...
      benefitSnapshot:{name:campaign.name,publicDisplay:{title:campaign.name}},authorizationSource:{kind:'member_gift_campaign',campaignVersionId:campaign.id,jobId},reason:'已审核发布的活动发放',issuanceIdempotencyKey:`member-gift:${jobId}`,issuanceFingerprint:hash({jobId,campaignVersionId:campaign.id})})
    const dessertBenefit=campaign.rule.dessertProductId?await new BenefitRepository(this.tx).issue({customerId,benefitCode:campaign.code,quantity:job.quantity,couponCalendarVersionId:campaign.rule.couponCalendarVersionId,benefitType:'gift_product',allowedProductIds:[campaign.rule.dessertProductId],benefitSnapshot:{name:`${campaign.name} · 小甜点`,publicDisplay:{title:`${campaign.name} · 小甜点`}},authorizationSource:{kind:'member_gift_campaign',campaignVersionId:campaign.id,jobId,pairedWithBenefitId:benefit.id},reason:'组合赠送的小甜点权益；实际领取另行核销',issuanceIdempotencyKey:`member-gift-dessert:${jobId}`,issuanceFingerprint:hash({jobId,campaignVersionId:campaign.id,dessertProductId:campaign.rule.dessertProductId})}):null
