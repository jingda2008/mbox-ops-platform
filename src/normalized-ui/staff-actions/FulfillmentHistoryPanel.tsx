...
export function FulfillmentHistoryPanel({api,kind,onOpenItem}:{api:StaffActionsApiPort;kind:'prepared'|'delivered';onOpenItem?(itemId:string):void}) {
 ...
 return <section className="staff-fulfillment-history" aria-label={kind==='prepared'?'我的历史制作':'我的历史送达'}>
  ...
  {data?.orders.map(order=><article className="staff-action-card" key={order.id}><header><strong>{order.tableCode}</strong><small>{order.publicId}</small></header>{order.items.map(item=><div key={item.id}><strong>{item.name} ×{item.quantity}</strong>{item.note&&<p>{item.note}</p>}{onOpenItem&&<button type="button" onClick={()=>onOpenItem(item.id)}>商品处理</button>}<p>{kind==='prepared'?item.preparedBy:item.deliveredBy} · {((kind==='prepared'?item.preparedAt:item.deliveredAt) ? new Date((kind==='prepared'?item.preparedAt:item.deliveredAt)!).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',hour12:false}) : '完成时间未留存')}</p></div>)}</article>)}
