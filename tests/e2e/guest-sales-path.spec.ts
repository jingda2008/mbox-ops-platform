...
test.describe('客人推荐销售路径', () => {
  ...
  test('快速选择重排原菜单，摇一摇只从同一候选池给出最多三次灵感', async ({ page }) => {
    ...
    const relaxedPrimary = page.locator('.menu-recommendation-option.is-primary')
    await expect(relaxedPrimary).toContainText('人气优选')
    const relaxedPrimaryName = await relaxedPrimary.locator('.menu-recommendation-option-title strong').innerText()
    const relaxedCards = await page.locator('.menu-recommendation-option-title strong').allInnerTexts()
    ...
    await expect(page.getByTestId('recommendation-updated-feedback')).toContainText('已按你的选择')
    const ritualPrimary = page.locator('.menu-recommendation-option.is-primary')
    await expect(ritualPrimary).toContainText('人气优选')
    await expect(ritualPrimary.locator('.menu-recommendation-option-title strong')).not.toHaveText(relaxedPrimaryName)
    await expect.poll(async () => page.locator('.menu-recommendation-option-title strong').allInnerTexts()).not.toEqual(relaxedCards)
