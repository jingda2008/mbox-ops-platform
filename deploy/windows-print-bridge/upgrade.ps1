...
try {
  ...
  if ($manifest.version -ne '1.0.3' -or $manifest.packageRevision -ne 'r2') { throw '升级包版本不正确。' }
  ...
  $operations = @{
    Validate = {
      ...
      if ([version]$Matches[1] -gt [version]'1.0.3') { throw '本包不会降级更新的版本。' }
      $same = $true
  ...
  $record = [pscustomobject]@{ Time=(Get-Date).ToString('o'); Package='1.0.3-r2'; Result=$result; Backup=$backup; BackendHeartbeat='not-verified'; PhysicalPrinting='not-verified' }
  ...
  if ($result.State -eq 'failed') {
    ...
    $message = "升级未完成。阶段：$($result.Stage)" + [Environment]::NewLine + $recoveryText + [Environment]::NewLine + '若处于idle阶段，请等票据出完后再试；持续失败请联系运维。' + [Environment]::NewLine + "备份：$backup"
    $exitCode = 1
  } else {
    $message = '打印程序已核验为1.0.3，服务正在运行。' + [Environment]::NewLine + '原配对和打印机配置保留。还需后台确认新心跳为1.0.3，并核对两台实体票。' + [Environment]::NewLine + "备份：$backup"
    $exitCode = 0
  }
