...
export function ItemAfterSalesPanel({itemId,employeeId,onClose,onChanged}:{itemId:string;employeeId:string;onClose():void;onChanged():void}){
  ...
  return <dialog ref={dialog} aria-label="商品停止与退款" className="staff-item-after-sales" onCancel={event=>{event.preventDefault();event.stopPropagation();onClose()}}>
    ...
    {refreshDelayed&&<p role="status">状态更新暂时延迟，可刷新查看；原处理结果保留。</p>}
    ...
    {!data?<p>正在读取原订单…</p>:<>
      ...
      {data.cases.map(current=>{
        ...
        const selection=held.slice(0,Number(count)).map(unit=>unit.id)
        ...
        return <article key={current.caseId}>
          ...
          {current.unconfirmedNoticeCount>0&&<section aria-label="岗位通知待确认">
            ...
            {current.notices.map(item=><p key={item.id}>{item.stationCode==='kitchen'?'后厨':'吧台'}：{item.instruction} · {item.printState==='attention'?'打印异常，请联系岗位':item.printState==='printed'?'打印程序已报完成，待岗位知悉':'等待打印，仍可直接联系岗位'}</p>)}
          ...
          {(current.canDisposeMade??current.status==='approved')&&held.length>0&&(data.canReceive||data.canRecordUsed)&&<fieldset disabled={locked}>
            ...
            {data.canReceive&&<><label><input type="checkbox" checked={received[current.caseId]??false} onChange={event=>setReceived(value=>({...value,[current.caseId]:event.target.checked}))}/>已实际收回且未开封</label><button disabled={!validCount||!received[current.caseId]} onClick={()=>void action(current.caseId,'physical',{unitIds:selection,disposition:'returned_unopened',unopenedReceived:true})}>确认收回入库</button></>}
