...
test('customer menu categories come from the editable backend hierarchy and reveal a configured second level only when needed', async () => {
  ...
  assert.match(orderLogic, /function menuCategoryState\(products, selectedTopCategory, selectedSubcategory\)/)
...
test('customer-only reservations stay executable, performances use the public schedule, and store contact is opt-in configured', async () => {
  ...
  assert.match(profileLogic, /openReservations\(\)\s*\{[^}]*wx\.switchTab\(\{ url: '\/pages\/reservations\/index' \}\)/)
  assert.doesNotMatch(profileLogic, /openReservations\(\)\s*\{[^}]*requireMembership/)
  assert.doesNotMatch(orderLogic, /requireMembershipLogin/)
  assert.match(orderLogic, /wx\.scanCode\(\{/)
  assert.doesNotMatch(reservationLogic, /getMiniBootstrap|membershipRequired|redirectToMembershipLogin|goMembershipLogin/)
  assert.doesNotMatch(reservationLogic, /getWechatNotificationAuthorizations|[gG]etWechatMemberServiceNotificationAuthorizations/)
  ...
  assert.doesNotMatch(reservationLogic, /getWechatNotificationPrompt\('(activity_registration|order_checkout|member_card|coupon_open)'\)/)
  ...
  assert.doesNotMatch(reservationView, /membership-gate|请先登录会员|登录后才能预约/)
...
test('subscription messages are requested from customer actions, not a settings-page application flow', async () => {
  ...
  const reservationOptions = helperModule.exports.buildReservationSubscriptionPresentation([
    { apiKind: 'reservation_performance', notificationType: 'reservation_performance_revised', templateId: 'reservation-template' },
    { apiKind: 'member_service', notificationType: 'member_benefit_issued', templateId: 'benefit-template' },
    { apiKind: 'loyalty', notificationType: 'loyalty_points_credited', templateId: 'points-template' },
  ])
  assert.deepEqual(Array.from(reservationOptions, (item) => item.notificationType), ['reservation_performance_revised'])
  assert.deepEqual(Array.from(helperModule.exports.RESERVATION_SUCCESS_SUBSCRIBE_TYPES), ['reservation_performance_revised'])
