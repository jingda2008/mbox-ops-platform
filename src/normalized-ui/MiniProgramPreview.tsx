...
function HomePreview({ tableReady, onOrder, onActivity }: { tableReady: boolean; onOrder(): void; onActivity(): void }) {
  ...
  return <>
    ...
    <section className="mini-section mini-home-discovery">
      ...
      <button type="button" className="mini-home-activity-card" onClick={onActivity}><i className="mini-home-activity-art"><Music2 /><small>SUPERHIGH</small></i><span className="mini-home-activity-copy"><small>超嗨 · 本期精选</small><strong>当期活动</strong><span>已发布、仍可参与</span><b>查看活动详情</b></span><ChevronRight /></button>
      <button type="button" className="mini-home-activity-card mini-home-story-card"><i className="mini-home-activity-art mini-home-story-art"><img src={miniLogo} alt="M-BOX 1999" /><small>M-BOX STORY</small></i><span className="mini-home-activity-copy"><small>品牌故事 · 城市现场</small><strong>从1999开始</strong><span>关于上海、现场与 M-BOX 的真实故事</span><b>阅读故事</b></span><ChevronRight /></button>
    </section>
...
function CommunityPreview({ open, onOpen, partySize, onPartySize }: { open: 'paid' | 'free' | null; onOpen(value: 'paid' | 'free' | null): void; partySize: number; onPartySize(value: number): void }) {
  ...
  return <><Band eyebrow="SUPERHIGH" title="今晚之外，继续相遇" copy="音乐、城市与真实的人；先看详情，再决定是否参加。" />
    <section className="mini-section mini-community-list"><header><strong>近期活动</strong><span>1 场可报名 · 1 场待开放</span></header><article className="mini-event-card"><div className="mini-event-art"><Music2 /><span>仅30位</span></div><div><em>会员音乐夜</em><h2>歌手主场后的深夜小聚</h2><p>周六 23:20 · M-BOX陆家嘴店</p><footer><strong>¥168/人</strong><button type="button" onClick={() => onOpen('paid')}>详情<ChevronRight /></button></footer></div></article><article className="mini-event-card"><div className="mini-event-art is-walk"><MapPin /><span>免费</span></div><div><em>城市漫步</em><h2>陆家嘴夜景音乐散步</h2><p>周三 19:00 · 无需预付</p><footer><strong>免费</strong><button type="button" onClick={() => onOpen('free')}>详情<ChevronRight /></button></footer></div></article></section>
  </>
}
...
function ProfilePreview() {
  ...
  return <><Band eyebrow="MY M-BOX" title="你的今晚，都在这里" copy="订单、预约、活动和真正可使用的权益。" />
    <section className="mini-profile-identity"><img src={miniLogo} alt="M-BOX" /><span><strong>{member ? 'M-BOX 会员' : 'M-BOX 访客'}</strong><small>{member ? 'MBX-35648' : '勾选同意后即可确认加入会员'}</small></span><b>{member ? '会员中心' : '邀请入会'}</b></section>
    <section className="mini-profile-metrics"><button type="button"><i><Crown /></i><strong>{member ? '1,280' : '0'}</strong><span>积分<ChevronRight /></span></button><button type="button"><i><TicketCheck /></i><strong>{member ? '2' : '0'}</strong><span>优惠券<ChevronRight /></span></button><button type="button"><i><Zap /></i><strong>{member ? '360' : '0'}</strong><span>成长值<ChevronRight /></span></button><button type="button"><i><Gift /></i><strong>0</strong><span>余额<ChevronRight /></span></button></section>
    {!member && <section className="mini-section mini-join-invite"><small>INVITATION</small><h2>邀请加入 M-BOX 会员</h2><p>保存积分、到店偏好与门店发放的专属权益。</p><label><input type="checkbox" checked={agreed} onChange={(event) => setAgreed(event.target.checked)} />我已阅读并同意 <u>《用户服务协议》</u></label><button type="button" disabled={!agreed} onClick={() => setMember(true)}>确定入会</button></section>}
    <section className="mini-section mini-profile-services"><header><strong>我的服务</strong><span>点击进入</span></header><div><button type="button"><Martini /><span><b>我的订单</b><small>订单与桌账</small></span><ChevronRight /></button><button type="button"><CalendarDays /><span><b>我的预约</b><small>到店安排</small></span><ChevronRight /></button><button type="button"><UsersRound /><span><b>超嗨活动</b><small>1 场已报名</small></span><ChevronRight /></button><button type="button"><UserRound /><span><b>我的偏好</b><small>口味与服务</small></span><ChevronRight /></button></div></section>
    <section className="mini-section mini-registered-activities"><header><strong>已报名的超嗨活动</strong><span>更多活动 ›</span></header><button type="button"><UsersRound /><span><b>歌手主场后的深夜小聚</b><small>周六 23:20 · 2人</small></span><em>已报名<ChevronRight /></em></button></section>
  </>
}
