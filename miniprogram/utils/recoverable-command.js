...
function recoverableGuestCommand(operation, body, send) {
  ...
  const principal = wx.getStorageSync('mbox.wechat.identity.principal.v1') || {}
  ...
  try { key = key || wx.getStorageSync(storageKey) } catch (_error) { /* Same-runtime retry remains available. */ }
  if (typeof key !== 'string' || !key) key = randomId('guest-command')
  attempts.set(storageKey, key)
  try { wx.setStorageSync(storageKey, key) } catch (_error) { /* No business body is stored. */ }
  const clear = () => {
    attempts.delete(storageKey)
    try { wx.removeStorageSync(storageKey) } catch (_error) { /* Harmless stale key replays safely. */ }
  }
...
module.exports = { recoverableGuestCommand }
