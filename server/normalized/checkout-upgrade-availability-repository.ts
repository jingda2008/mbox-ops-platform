  ...
  const result=await this.tx.query<{available:boolean;inventory_known_and_sufficient:boolean;production_available:boolean}>(`
   ...
   SELECT (SELECT count(*) FROM products)=(SELECT count(*) FROM requested)
    ...
    NOT EXISTS(SELECT 1 FROM production WHERE window_id IS NULL)
      AND NOT EXISTS(SELECT 1 FROM capacity c WHERE c.required+COALESCE((SELECT sum(mbox.item_remaining_capacity_units(held.tenant_id,held.store_id,held.order_item_id,held.capacity_units)) FROM mbox.fulfillment_capacity_reservations held WHERE held.tenant_id=$1 AND held.store_id=$2 AND held.capacity_window_id=c.window_id AND held.status IN('reserved','active')),0)+mbox.remake_window_used_units($1,$2,c.window_id)>c.limit_units) AS production_available
