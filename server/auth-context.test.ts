...
describe('staff session', () => {
  it('signs and verifies bounded staff claims', () => {
    const token = signStaffSession({ actorId: 'emp-chen', storeId: 'mbox-lujiazui', issuedAt: now, expiresAt: now + 60_000 }, secret)
    expect(verifyStaffSession(token, secret, now)).toMatchObject({ actorId: 'emp-chen', storeId: 'mbox-lujiazui' })
  })

  it('rejects tampered and expired sessions', () => {
    const token = signStaffSession({ actorId: 'emp-chen', storeId: 'mbox-lujiazui', issuedAt: now, expiresAt: now + 1_000 }, secret)
  ...
  it('requires a production-grade signing secret', () => {
    expect(() => signStaffSession({ actorId: 'a', storeId: 's', issuedAt: now, expiresAt: now + 1 }, 'short')).toThrow(
      '会话密钥至少需要32个字符',
    )
  })
})
