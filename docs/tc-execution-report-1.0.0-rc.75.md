...
逐条登记：[tc-execution-register-1.0.0-rc.75.csv](tc-execution-register-1.0.0-rc.75.csv)
发布阻断：[tc-release-blockers-1.0.0-rc.75.csv](tc-release-blockers-1.0.0-rc.75.csv)

## 结论

本轮基线共 **213条**，没有删除或放宽原始预期。下表保留既有经营验收状态，但**不把历史通过自动等同于`1.0.0-rc.75`当前候选已经重新执行**：
