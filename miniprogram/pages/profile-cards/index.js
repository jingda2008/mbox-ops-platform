  ...
  async load(kind) {
    ...
    try {
      ...
      this.setData({
        ...
        projects: data.projects.map(item => Object.assign({}, item, { untilText: time(item.available_until), canApply: data.activeMember && (!item.require_social_conditions || item.social_conditions_met) && item.accepting_applications && !item.has_current_card && !item.has_pending_application && !held.has(item.id) && !pending.has(item.id), buttonText: item.has_current_card || held.has(item.id) ? '已持有' : item.has_pending_application || pending.has(item.id) ? '审核中' : item.require_social_conditions && !item.social_conditions_met ? '请先完成双条件核验' : item.accepting_applications ? '查看并申请' : '暂未开放' })),
        cards: data.cards.map(item => Object.assign({}, item, { stateText: item.require_social_conditions && !item.social_conditions_met && ['active','suspended'].includes(item.status) ? '加入条件未满足，权益暂不可用' : item.expired && ['active', 'suspended'].includes(item.status) ? '已到期' : labels[item.status] || '待核实', untilText: time(item.valid_until), canWithdraw: ['active', 'suspended'].includes(item.status) })),
