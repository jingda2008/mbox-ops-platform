...
test('production worker adapters are inherited as immutable read-only release evidence', async () => {
  ...
  assert.match(activate, /hash_worker_adapter_tree\(\)[\s\S]*find \. -mindepth 1 -print0[\s\S]*LC_ALL=C sort -z/)
  assert.match(activate, /printf 'file\\0%s\\0%s\\0%s\\0'/)
  assert.doesNotMatch(activate, /tar --sort=name/)
...
test('post-cutover evidence uses the external relay before activation can complete', async () => {
  ...
  assert.match(deploy, /report_sha=\$\(shasum -a 256 "\$\{relay_local\}\/\$\{report_name\}"/)
  assert.match(deploy, /staged_report="\$\{remote_release_dir\}\/\.\$\{report_name\}\.next"/)
  assert.match(deploy, /scp "\$\{scp_options\[@\]\}" "\$\{relay_local\}\/\$\{report_name\}"[\s\S]*"\$\{ssh_target\}:\$\{staged_report\}"/)
  assert.match(deploy, /sha256sum '\$\{staged_report\}'[\s\S]*mv -f '\$\{staged_report\}' '\$\{remote_release_dir\}\/\$\{report_name\}'/)
  const atomicReportPublish = deploy.indexOf("mv -f '${staged_report}'")
  assert.ok(atomicReportPublish > 0)

  const completionArchive = activate.indexOf('"${completion_evidence}"')
  const canonicalRelease = activate.indexOf('ln -sfn "${release_dir}" "${current_link}"', completionArchive)
  const completedState = activate.indexOf('release_state_transition "${state_file}" evidence_archived completed', canonicalRelease)
  const completeFlag = activate.indexOf('complete=1', completedState)
  assert.ok(completionArchive > 0 && completionArchive < canonicalRelease)
  assert.ok(canonicalRelease < completedState && completedState < completeFlag)
  assert.match(activate, /evidence_relay_wait_seconds=\$\{MBOX_EVIDENCE_RELAY_WAIT_SECONDS:-300\}/)
