...
test('tonight ordering keeps live service separate from recommendation and delegates ranking to the server', async () => {
  ...
  assert.match(orderLogic, /selectRecommendationAnswer\(event\)[\s\S]*?this\.recommend\('guided'\)/)
  assert.match(orderLogic, /getRecommendationConfiguration\(\)/)
  assert.match(orderLogic, /EMPTY_RECOMMENDATION_CONFIGURATION/)
  assert.doesNotMatch(orderLogic, /FALLBACK_RECOMMENDATION_CONFIGURATION/)
  assert.doesNotMatch(orderView, /记录今晚偏好|recommend-preference/)
  assert.ok(orderView.indexOf('class="category-grid"') < orderView.indexOf('class="search-box"'), 'category grid appears before optional search')
  ...
  assert.doesNotMatch(orderStyle, /\.product-detail-sheet\.is-bundle \.product-detail-hero/)
  assert.match(orderStyle, /\.product-detail-choice-options \{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/)
  assert.ok(orderView.indexOf('class="product-detail-scroll"') < orderView.indexOf('class="product-detail-footer"'), 'the fixed confirmation footer stays outside the scrolling bundle content')
  assert.match(orderView, /class="cart-line__selection"[\s\S]*?bindtap="editBundleUnitSelection"/)
  assert.match(orderLogic, /editBundleUnitSelection\(event\)[\s\S]*?applyBundleUnitSelection/)
  assert.match(orderLogic, /replaceBundleUnitSelection\(product\.productId,this\.data\.detailEditUnitIndex/)
  assert.match(orderLogic, /selectionSource:\s*'cart_plus'[\s\S]*?cartExpanded:\s*false[\s\S]*?checkoutConfirmVisible:\s*false/)
  assert.match(alipayOrderStyle, /\.product-detail-sheet\.is-bundle \.product-detail-hero \{ height:\s*320rpx/)
  assert.match(alipayOrderStyle, /\.product-detail-choice-options \{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/)
  ...
  assert.match(orderView, /disabled="\{\{!orderReady \|\| cartSyncing \|\| cartWritesFrozen\}\}">\{\{checkoutLocked \? '继续确认' : '提交订单'\}\}/)
  assert.doesNotMatch(orderView, /disabled="\{\{busy \|\| pendingPayment \|\| cartSyncing/)
...
test('customer menu categories come from the editable backend hierarchy and reveal a configured second level only when needed', async () => {
  ...
  assert.match(orderLogic, /function menuCategoryState\(products, selectedTopCategory, selectedSubcategory, connected = false\)/)
  assert.doesNotMatch(orderLogic, /code: 'recommendation', name: '今夜推荐'|includeRecommendations/)
  assert.match(orderLogic, /const leadingCategories = \[\{ code: 'all', name: '全部'/)
...
test('customer pages use Chinese release copy and the table cart is server-authoritative', async () => {
  ...
  assert.match(cartRepository, /new OrderRepository\(this\.transaction,this\.customerId\)\.assertCurrentOrderable/)
...
test('subscription messages are requested from customer actions, not a settings-page application flow', async () => {
  ...
  assert.match(subscriptionSource, /buildActivitySubscriptionPresentation/)
  assert.match(subscriptionSource, /activity_performance_starting/)
  assert.match(subscriptionSource, /activity_schedule_changed/)
  assert.match(subscriptionSource, /4l6FLtKAkhYAb2v45G0NxaGAXmDsyi2gsD0F7H0h2Ag/)
  assert.match(subscriptionSource, /GQl7s-_G7gsUMliU7o_g3bstB9yXO6B2pctz1rwJMkg/)
  assert.match(activityLogic, /buildActivitySubscriptionPresentation/)
  assert.match(activityLogic, /ACTIVITY_REGISTRATION_SUBSCRIBE_TYPES/)
  ...
  assert.match(orderLogic, /extractPromptPresentation\(await getWechatNotificationPrompt\(\x27order_checkout\x27\)\)/)
  assert.match(orderLogic, /extractPromptPresentation\(await getWechatNotificationPrompt\(\x27order_selection\x27\)\)/)
  assert.match(orderLogic, /async addProduct\(event\)[\s\S]{0,900}?commitProductAdd\(productId/)
  assert.match(orderLogic, /async commitProductAdd\(productId,[\s\S]{0,900}?offerOrderNotifications\('order_selection', tableRequest\)/)
  assert.match(orderLogic, /async confirmBundleProduct\(\)[\s\S]{0,900}?commitProductAdd\(product\.productId/)
  ...
  assert.match(orderView, /class="checkout-confirm__close compact-text-action"[\s\S]{0,220}?bindtap="closeCheckoutConfirm"[\s\S]{0,80}?>关闭<\/button>/)
  ...
  assert.match(orderStyle, /\.checkout-confirm__close-slot\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?top:\s*-2rpx;[\s\S]*?right:\s*18rpx;[\s\S]*?width:\s*88rpx;[\s\S]*?height:\s*88rpx;/)
  assert.match(orderStyle, /\.checkout-confirm__close\s*\{\s*color:\s*#62594f;/)
  ...
  assert.match(orderLogic, /failureCode === 'network_rejected'/)
  assert.match(orderLogic, /当前网络未通过支付验证/)
  assert.match(orderLogic, /扫码、POS 或现金收款/)
