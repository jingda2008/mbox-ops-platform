...
test('config templates use explicit integration modes without secrets', () => {
  ...
  assert.match(validation, /MBOX_QUANTITY_AFTER_SALES_ENABLED=false/)
  assert.match(validation, /MBOX_KITCHEN_BATCH_BOARD_ENABLED=false/)
  assert.match(production, /MBOX_KITCHEN_BATCH_BOARD_ENABLED=false/)
  assert.match(production, /MBOX_QUANTITY_AFTER_SALES_ENABLED=false/)
  ...
  assert.match(production, /DATABASE_URL=<postgresql-url-with-explicit-restricted-login>/)
  assert.doesNotMatch(production, /^(ADMIN_DATABASE_URL|BACKUP_DATABASE_URL|PGPASSFILE|PGSERVICEFILE)=/m)
