...
const {
  ...
  recordAlipayNotificationAuthorization,
  enrollMembership,
...
function memberCardPreviewLevel() {
  try {
    const options = runtime.getEnterOptionsSync ? runtime.getEnterOptionsSync() : runtime.getLaunchOptionsSync()
...
function clearMemberLocalCache() {
  runtime.removeStorageSync('mbox.member.avatarUrl')
  runtime.removeStorageSync('mbox.member.displayName')
}
...
Page({
  ...
  async load() {
    ...
    try {
      ...
      const preferenceView = customerPreferenceView(results[10])
      ...
      this.setData({
        ...
        alipayNotificationAuthorizations: (results[7] && results[7].authorizations) || [],
        alipayNotificationPromptOptions: mergeAlipayNotificationPromptOptions(
          results[8] && results[8].authorizations,
          results[9] && results[9].authorizations,
        ),
        alipaySubscriptionPresentationOptions: extractPromptPresentation(results[8]),
        alipayMemberCardPresentationOptions: extractPromptPresentation(results[9]),
        ...
        displayName: runtime.getStorageSync('mbox.member.displayName') || '',
        avatarUrl: runtime.getStorageSync('mbox.member.avatarUrl') || '',
        ...preferenceView,
      })
      rememberPresentationOptions('coupon_open', extractPromptPresentation(results[8]))
      rememberPresentationOptions('member_card', extractPromptPresentation(results[9]))
    } catch (error) {
      const message = String((error && error.message) || '')
      ...
      this.setData({
        ...
        expiryNotificationOption:null,alipayNotificationAuthorizations:[],alipayNotificationPromptOptions:[],showRedemptions:false,
        preferenceFacts:[],preferenceSources:[],preferenceSourceCount:0,preferenceActiveCount:0,
      })
    }
  },
  ...
  continuePendingMembershipAction() {
    ...
    runtime[destination[1]]({ url: destination[0] })
  },
  ...
  onAgreementChange(event) {
    ...
    if (Array.isArray(detail.value)) {
      this.setData({ agreedToPolicies: detail.value.indexOf('agree') >= 0 })
      return
    }
    if (typeof detail.checked === 'boolean') {
      this.setData({ agreedToPolicies: detail.checked })
      return
    }
    this.setData({ agreedToPolicies: !this.data.agreedToPolicies })
  },

  remindAgreement() {
    runtime.showToast({ title: '请先勾选并同意会员协议', icon: 'none' })
  },
  ...
  async logoutMember() {
    ...
    try {
      try {
        await logoutAlipayIdentity()
      } catch (_error) {
        // 服务端注销失败时仍清除本地会话，避免卡在半登录态。
      }
      clearMemberLocalCache()
      this.pendingMembershipAction = ''
      runtime.removeStorageSync('mbox.membership.enroll.attempt.v1')
      runtime.removeStorageSync('mbox.membership.recovery.attempt.v1')
      ...
      runtime.showToast({ title: '已退出登录', icon: 'success', duration: 1200 })
      await this.load()
    } finally {
  ...
  async quickLoginAndEnroll(event) {
    ...
    try {
      ...
      runtime.showToast({ title: '登录成功', icon: 'success', duration: 1200 })
      await this.load()
      this.continuePendingMembershipAction()
    } catch (error) {
      this.pendingMembershipAction = ''
      const friendly = membershipEnrollErrorMessage(error, '登录暂时没有完成')
      this.setData({ error: friendly })
      runtime.showToast({ title: friendly, icon: 'none' })
    } finally {
  ...
  showMembershipTerms() {
    runtime.navigateTo({ url: '/pages/membership-terms/index?source=mini_profile&action=view' })
  },
  ...
  async enableExpiryReminder() {
    ...
    if (!option || typeof runtime.requestSubscribeMessage !== 'function') return
    try {
      const options = mergeAlipayNotificationPromptOptions(
        [Object.assign({}, option, { apiKind: 'loyalty' })],
        this.data.alipayMemberCardPresentationOptions,
        this.data.alipaySubscriptionPresentationOptions,
      )
  ...
  async recoverMembership(event) {
    ...
    try {
      ...
      if (result.status === 'completed') {
        this.setData({ loginSheetVisible: false })
        runtime.showToast({ title: '会员已找回', icon: 'success' })
        await this.load()
      }
    } catch (error) {
      const friendly = membershipEnrollErrorMessage(error, '历史会员找回暂时没有完成')
      this.setData({ recoveryMessage: friendly })
      runtime.showToast({ title: friendly, icon: 'none', duration: 3500 })
    } finally {
  ...
  async submitCustomerPreference() {
    ...
    try {
      ...
      runtime.showToast({ title: '偏好已记录', icon: 'success' })
    } catch (error) {
  ...
  async correctCustomerPreference(event) {
    ...
    try {
      ...
      this.applyCustomerPreferenceSnapshot(snapshot)
      runtime.showToast({ title: '已记录纠正', icon: 'success' })
    } catch (error) {
  ...
  async withdrawCustomerPreference(event) {
    ...
    try {
      ...
      this.applyCustomerPreferenceSnapshot(snapshot)
      runtime.showToast({ title: '已撤回', icon: 'success' })
    } catch (error) {
  ...
  openMemberContentCard(event) {
    ...
    if (!targetPath) {
      runtime.showToast({ title: '该内容暂不支持跳转', icon: 'none' })
      return
    }
    if (CONTENT_CARD_TAB_TARGETS.has(targetPath)) {
      runtime.switchTab({ url: targetPath })
      return
    }
    runtime.navigateTo({ url: targetPath })
  },
  ...
  async withdrawRestriction(event) {
    ...
    try {
      ...
      runtime.showToast({ title: '已恢复推荐', icon: 'success' })
    } catch (error) {
  ...
  async requestBenefitUse(event) {
    ...
    try {
      ...
      runtime.showModal({ title: '已经申请使用', content: '权益已经为本桌暂留，需由服务人员完成实际交付和核销。', showCancel: false })
      await this.load()
  ...
  async redeemItem(event) {
    ...
    if (item.requiresTableSession && !this.data.hasTableContext) {
      return runtime.showModal({ title: '请先扫码入座', content: '该兑换需要有效桌次，入座扫描桌码后才能确认。', showCancel: false })
    }
    const confirmed = await new Promise((resolve) => runtime.showModal({
    ...
    try {
      ...
      runtime.showModal({ title: '兑换已确认', content: '积分已扣减并生成交付记录；需要制作的商品现已进入对应出品工位。', showCancel: false })
      await this.load()
  ...
  openReservations() {
    runtime.switchTab({ url: '/pages/reservations/index' })
  },
    ...
    try {
      ...
      this.setData({ registrations, registrationError: '' }, () => {
        ...
        runtime.pageScrollTo({
          ...
          duration: 320,
          fail: () => runtime.showToast({ title: '已报名活动正在显示，请稍后重试', icon: 'none' }),
        })
      })
    } catch (error) {
  ...
  openSuperhighTab() { runtime.switchTab({ url: '/pages/community/index' }) },
  openPoints() {
    if (!this.requireMembership('points')) return
    runtime.navigateTo({ url: '/pages/points/index' })
  },
  openOrders() {
    if (!this.requireMembership('orders')) return
    runtime.navigateTo({ url: '/pages/account/index' })
  },
  ...
  openContact() {
    ...
    runtime.navigateTo({ url: '/pages/profile-contact/index' })
  },
  async openCoupons() {
    if (!this.requireMembership('coupons')) return
    await requestAlipaySubscription(this.data.alipaySubscriptionPresentationOptions || [])
    runtime.navigateTo({ url: '/pages/profile-coupons/index' })
  },
  openNotificationSettings() {
    if (!this.requireMembership('notifications')) return
    runtime.navigateTo({ url: '/pages/profile-notifications/index' })
  },
  openCommunity(event) {
    if (!this.requireMembership()) return
