...
interface Campaign{highlights:Record<string,string>;id:string;code:string;name:string;version:number;status:string;calendar_code:string;calendar_version:number;card_project_name:string|null;created_by_employee_id:string;approved_by_employee_id:string|null;products:Array<{product_id:string;name:string;unit_cost_minor:string}>;rule:{dessertProductId?:string|null;highlightMetrics?:string[];pricingKind:'free'|'fixed_price';fixedPriceMinor:number|null;stackingVersionId:string|null;trigger:string;quantityPerCustomer:number;maximumQuantity:number;maximumDailyQuantity:number;maximumCostMinor:number;maximumDailyCostMinor:number;maximumUnitCostMinor:number;availableFrom:string;availableUntil:string;budgetDateBasis:string;budgetDayStartMinute:number;couponCalendarVersionId:string;audience:{minimumTier:string|null;cardCodes:string[];cardMatch:string;tierAndCards:string}}}
...
export function MemberGiftCampaignPanel({api,auth}:{api:NormalizedApiClient;auth:StaffAuthView}){
  ...
  const [dessert,setDessert]=useState<Option[]>([]),[highlights,setHighlights]=useState(['issued','redeemed','remaining'])
  ...
  return <section className="member-gift-panel" aria-label="入卡礼与定向发券"><h3>入卡礼与定向发券</h3><p>卡审核、赠券发放、消息送达分别记录。活动停发不删除已发券；成本未知不自动放行。</p>
    ...
    {canEdit&&<details><summary>新建活动版本</summary><fieldset disabled={busy}><div className="gift-form-grid">
      ...
      <Selector api={api} kind="products" label="组合赠送小甜点（选填，选定后每人每活动限一套）" value={dessert} onChange={setDessert} disabled={busy}/><p>选择甜点后，两份权益一起发放，成本预算包含券商品最高成本和甜点成本；甜点在会员券包单独核销。</p>{['issued','redeemed','remaining','cost'].map((key,i)=><label key={key}><input type="checkbox" checked={highlights.includes(key)} onChange={e=>setHighlights(e.target.checked?[...highlights,key]:highlights.filter(k=>k!==key))}/>统计：{['已发套数','甜点已核销份数','剩余发放名额','承诺成本'][i]}</label>)}
